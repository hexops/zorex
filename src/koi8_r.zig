...
pub const EncKOI8_R_ToLowerCaseTable: [256]OnigUChar = [256]OnigUChar{
...
pub const EncKOI8_R_CtypeTable: [256]c_ushort = [256]c_ushort{
    ...
    @bitCast(c_ushort, @truncate(c_short, @as(c_int, 160))),
    @bitCast(c_ushort, @truncate(c_short, @as(c_int, 160))),
    @bitCast(c_ushort, @truncate(c_short, @as(c_int, 4256))),
...
pub fn koi8_r_mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    var flag = arg_flag;
    var pp = arg_pp;
    var end = arg_end;
    var lower = arg_lower;
    var p: [*c]const OnigUChar = pp.?.*;
    if ((((flag) & @bitCast(c_uint, (@as(c_int, 1)))) == @bitCast(c_uint, @as(c_int, 0))) or (@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 128))) lower.?.* = EncKOI8_R_ToLowerCaseTable[p.?.*] else lower.?.* = p.?.*;
    (pp.?.*) += 1;
    return 1;
}
pub fn koi8_r_is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    if (code < @bitCast(c_uint, @as(c_int, 256))) return @boolToInt(((@bitCast(c_int, @as(c_uint, EncKOI8_R_CtypeTable[code])) & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), (ctype)))) != @as(c_int, 0))) else return 0;
    return 0;
}
...
pub fn koi8_r_apply_all_case_fold(arg_flag: OnigCaseFoldType, arg_f: OnigApplyAllCaseFoldFunc, arg_arg: ?*c_void) callconv(.C) c_int {
    var flag = arg_flag;
    var f = arg_f;
    var arg = arg_arg;
...
pub fn koi8_r_get_case_fold_codes_by_str(arg_flag: OnigCaseFoldType, arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_items: [*c]OnigCaseFoldCodeItem) callconv(.C) c_int {
    var flag = arg_flag;
    var p = arg_p;
    var end = arg_end;
    var items = arg_items;
...
pub fn ENC_KOI8_R_TO_LOWER_CASE(c: anytype) callconv(.Inline) @TypeOf(EncKOI8_R_ToLowerCaseTable[c]) {
    return EncKOI8_R_ToLowerCaseTable[c];
}
pub fn ENC_IS_KOI8_R_CTYPE(code: anytype, ctype: anytype) callconv(.Inline) @TypeOf((EncKOI8_R_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0) {
    return (EncKOI8_R_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0;
}
