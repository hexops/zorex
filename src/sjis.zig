...
pub const EncLen_SJIS: [256]c_int = [256]c_int{
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
...
pub const SJIS_CAN_BE_TRAIL_TABLE: [256]u8 = [256]u8{
...
pub fn is_valid_mbc_string(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    ...
    while (p < end) {
        if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 128)) {
            p += 1;
        } else if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 161)) {
            if ((@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 160)) or (@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 128))) return 0;
            p += 1;
            if (p >= end) return 0;
            if (((@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 64)) or (@bitCast(c_int, @as(c_uint, p.?.*)) > @as(c_int, 252))) or (@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 127))) return 0;
            p += 1;
        } else if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 224)) {
            p += 1;
        } else if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 253)) {
            p += 1;
            if (p >= end) return 0;
            if (((@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 64)) or (@bitCast(c_int, @as(c_uint, p.?.*)) > @as(c_int, 252))) or (@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 127))) return 0;
            p += 1;
...
pub fn code_to_mbclen(arg_code: OnigCodePoint) callconv(.C) c_int {
    var code = arg_code;
    if (code < @bitCast(c_uint, @as(c_int, 256))) {
        if (EncLen_SJIS[@intCast(c_uint, @bitCast(c_int, code))] == @as(c_int, 1)) return 1;
    } else if (code < @bitCast(c_uint, @as(c_int, 65536))) {
        if (EncLen_SJIS[@intCast(c_uint, (@bitCast(c_int, (code >> @intCast(@import("std").math.Log2Int(OnigCodePoint), 8))) & @as(c_int, 255)))] == @as(c_int, 2)) return 2;
    }
    return -@as(c_int, 400);
}
pub fn mbc_to_code(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) OnigCodePoint {
    ...
    len = (&OnigEncodingSJIS).*.mbc_enc_len.?(p);
...
pub fn code_to_mbc(arg_code: OnigCodePoint, arg_buf: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if ((code & @bitCast(c_uint, @as(c_int, 65280))) != @bitCast(c_uint, @as(c_int, 0))) (blk: {
        const ref = &p;
        const tmp = ref.*;
        ref.* += 1;
        break :blk tmp;
...
pub fn mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if (@bitCast(c_int, @as(c_uint, (p).?.*)) < @as(c_int, 128)) {
        ...
        var i: c_int = undefined;
        var len: c_int = (&OnigEncodingSJIS).*.mbc_enc_len.?(p);
        {
            i = 0;
            while (i < len) : (i += 1) {
                (blk: {
...
pub fn left_adjust_char_head(arg_start: [*c]const OnigUChar, arg_s: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    ...
    p = s;
    if (SJIS_CAN_BE_TRAIL_TABLE[(p.?.*)] != 0) {
        while (p > start) {
            if (!(EncLen_SJIS[(blk: {
                    const ref = &p;
                    ref.* -= 1;
...
pub fn is_allowed_reverse_match(arg_s: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    ...
    var end = arg_end;
    const c: OnigUChar = s.?.*;
    return (if (@bitCast(c_int, @as(c_uint, SJIS_CAN_BE_TRAIL_TABLE[(c)])) != 0) @as(c_int, 0) else @as(c_int, 1));
}
pub const CR_Hiragana: [3]OnigCodePoint = [3]OnigCodePoint{
    @bitCast(OnigCodePoint, @as(c_int, 1)),
    @bitCast(OnigCodePoint, @as(c_int, 33439)),
    @bitCast(OnigCodePoint, @as(c_int, 33521)),
};
pub const CR_Katakana: [9]OnigCodePoint = [9]OnigCodePoint{
    @bitCast(OnigCodePoint, @as(c_int, 4)),
    @bitCast(OnigCodePoint, @as(c_int, 166)),
    @bitCast(OnigCodePoint, @as(c_int, 175)),
    @bitCast(OnigCodePoint, @as(c_int, 177)),
    @bitCast(OnigCodePoint, @as(c_int, 221)),
    @bitCast(OnigCodePoint, @as(c_int, 33600)),
    @bitCast(OnigCodePoint, @as(c_int, 33662)),
    @bitCast(OnigCodePoint, @as(c_int, 33664)),
    @bitCast(OnigCodePoint, @as(c_int, 33686)),
};
...
pub fn property_name_to_ctype(arg_enc: OnigEncoding, arg_p: [*c]OnigUChar, arg_end: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if (@bitCast(c_ulong, @as(c_long, len)) < (@sizeOf([32]u8) -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))))) {
        ...
        pc = onigenc_sjis_lookup_property_name(&q, @bitCast(usize, @as(c_long, len)));
...
pub fn get_ctype_code_range(arg_ctype: OnigCtype, arg_sb_out: [*c]OnigCodePoint, arg_ranges: [*c][*c]const OnigCodePoint) callconv(.C) c_int {
    ...
    if (ctype <= @bitCast(c_uint, ONIGENC_CTYPE_ASCII)) {
        ...
        ctype -%= @bitCast(c_uint, (ONIGENC_CTYPE_ASCII + @as(c_int, 1)));
        if (ctype >= @bitCast(OnigCtype, @truncate(c_uint, (@sizeOf([2][*c]const OnigCodePoint) / @sizeOf([*c]const OnigCodePoint))))) return -@as(c_int, 6);
...
pub fn SJIS_ISMB_FIRST(byte: anytype) callconv(.Inline) @TypeOf(EncLen_SJIS[byte] > 1) {
    return EncLen_SJIS[byte] > 1;
}
pub fn SJIS_ISMB_TRAIL(byte: anytype) callconv(.Inline) @TypeOf(SJIS_CAN_BE_TRAIL_TABLE[byte]) {
    return SJIS_CAN_BE_TRAIL_TABLE[byte];
}
