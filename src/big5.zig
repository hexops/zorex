...
pub const EncLen_BIG5: [256]c_int = [256]c_int{
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
...
pub fn big5_mbc_enc_len(arg_p: [*c]const OnigUChar) callconv(.C) c_int {
    var p = arg_p;
    return EncLen_BIG5[p.?.*];
}
pub fn big5_code_to_mbclen(arg_code: OnigCodePoint) callconv(.C) c_int {
    var code = arg_code;
    if ((code & @bitCast(c_uint, (~@as(c_int, 65535)))) != @bitCast(c_uint, @as(c_int, 0))) return -@as(c_int, 400);
    if ((code & @bitCast(c_uint, @as(c_int, 65280))) != @bitCast(c_uint, @as(c_int, 0))) {
        if (EncLen_BIG5[@intCast(c_uint, (@bitCast(c_int, (code >> @intCast(@import("std").math.Log2Int(OnigCodePoint), 8))) & @as(c_int, 255)))] == @as(c_int, 2)) return 2;
    } else {
        if (EncLen_BIG5[@intCast(c_uint, @bitCast(c_int, (code & @bitCast(c_uint, @as(c_int, 255)))))] == @as(c_int, 1)) return 1;
    }
    return -@as(c_int, 400);
}
pub fn is_valid_mbc_string(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    ...
    while (p < end) {
        if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 128)) {
            ...
            p += 1;
            if (p >= end) return 0;
            if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 64)) return 0;
            if ((@bitCast(c_int, @as(c_uint, p.?.*)) > @as(c_int, 126)) and (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 161))) return 0;
...
pub fn big5_mbc_to_code(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) OnigCodePoint {
    var p = arg_p;
    var end = arg_end;
    return onigenc_mbn_mbc_to_code((&OnigEncodingBIG5), p, end);
}
pub fn big5_code_to_mbc(arg_code: OnigCodePoint, arg_buf: [*c]OnigUChar) callconv(.C) c_int {
    var code = arg_code;
    var buf = arg_buf;
    return onigenc_mb2_code_to_mbc((&OnigEncodingBIG5), code, buf);
}
pub fn big5_mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    var flag = arg_flag;
    var pp = arg_pp;
    var end = arg_end;
    var lower = arg_lower;
    return onigenc_mbn_mbc_case_fold((&OnigEncodingBIG5), flag, pp, end, lower);
}
pub fn big5_is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    return onigenc_mb2_is_code_ctype((&OnigEncodingBIG5), code, ctype);
}
pub const BIG5_CAN_BE_TRAIL_TABLE: [256]u8 = [256]u8{
...
pub fn big5_left_adjust_char_head(arg_start: [*c]const OnigUChar, arg_s: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var start = arg_start;
    var s = arg_s;
    ...
    p = s;
    if (BIG5_CAN_BE_TRAIL_TABLE[(p.?.*)] != 0) {
        while (p > start) {
            if (!(EncLen_BIG5[(blk: {
                    const ref = &p;
                    ref.* -= 1;
    ...
    len = (&OnigEncodingBIG5).*.mbc_enc_len.?(p);
...
pub fn big5_is_allowed_reverse_match(arg_s: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    var s = arg_s;
    var end = arg_end;
    const c: OnigUChar = s.?.*;
    return (if (@bitCast(c_int, @as(c_uint, BIG5_CAN_BE_TRAIL_TABLE[(c)])) != 0) @as(c_int, 0) else @as(c_int, 1));
}
...
pub fn BIG5_ISMB_FIRST(byte: anytype) callconv(.Inline) @TypeOf(EncLen_BIG5[byte] > 1) {
    return EncLen_BIG5[byte] > 1;
}
pub fn BIG5_ISMB_TRAIL(byte: anytype) callconv(.Inline) @TypeOf(BIG5_CAN_BE_TRAIL_TABLE[byte]) {
    return BIG5_CAN_BE_TRAIL_TABLE[byte];
}
