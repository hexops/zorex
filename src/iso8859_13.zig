...
pub const EncISO_8859_13_ToLowerCaseTable: [256]OnigUChar = [256]OnigUChar{
...
pub const EncISO_8859_13_CtypeTable: [256]c_ushort = [256]c_ushort{
...
pub fn mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if ((((flag) & @bitCast(c_uint, (@as(c_int, 1)))) == @bitCast(c_uint, @as(c_int, 0))) or (@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 128))) lower.?.* = EncISO_8859_13_ToLowerCaseTable[p.?.*] else lower.?.* = p.?.*;
    (pp.?.*) += 1;
    return 1;
}
pub fn is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    if (code < @bitCast(c_uint, @as(c_int, 256))) return @boolToInt(((@bitCast(c_int, @as(c_uint, EncISO_8859_13_CtypeTable[code])) & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), (ctype)))) != @as(c_int, 0))) else return 0;
    return 0;
}
...
pub fn ENC_ISO_8859_13_TO_LOWER_CASE(c: anytype) callconv(.Inline) @TypeOf(EncISO_8859_13_ToLowerCaseTable[c]) {
    return EncISO_8859_13_ToLowerCaseTable[c];
}
pub fn ENC_IS_ISO_8859_13_CTYPE(code: anytype, ctype: anytype) callconv(.Inline) @TypeOf((EncISO_8859_13_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0) {
    return (EncISO_8859_13_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0;
}
