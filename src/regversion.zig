...
var s: [12]u8 = @import("std").mem.zeroes([12]u8);
pub fn onig_version() callconv(.C) [*c]const u8 {
    _ = s;
    _ = __builtin___snprintf_chk(&s, @sizeOf([12]u8), @as(c_int, 0), __builtin_object_size(@ptrCast(?*const c_void, &s), if (@as(c_int, 2) > @as(c_int, 1)) @as(c_int, 1) else @as(c_int, 0)), "%d.%d.%d", @as(c_int, 6), @as(c_int, 9), @as(c_int, 6));
    return &s;
}
var s_1: [58]u8 = @import("std").mem.zeroes([58]u8);
pub fn onig_copyright() callconv(.C) [*c]const u8 {
    _ = s_1;
    _ = __builtin___snprintf_chk(&s_1, @sizeOf([58]u8), @as(c_int, 0), __builtin_object_size(@ptrCast(?*const c_void, &s_1), if (@as(c_int, 2) > @as(c_int, 1)) @as(c_int, 1) else @as(c_int, 0)), "Oniguruma %d.%d.%d : Copyright (C) 2002-2018 K.Kosako", @as(c_int, 6), @as(c_int, 9), @as(c_int, 6));
    return &s_1;
}
...
pub fn ONIGENC_IS_MBC_WORD(enc: anytype, s_2: anytype, end: anytype) callconv(.Inline) @TypeOf(ONIGENC_IS_CODE_WORD(enc, ONIGENC_MBC_TO_CODE(enc, s_2, end))) {
    return ONIGENC_IS_CODE_WORD(enc, ONIGENC_MBC_TO_CODE(enc, s_2, end));
}
pub fn ONIGENC_IS_MBC_WORD_ASCII(enc: anytype, s_2: anytype, end: anytype) callconv(.Inline) @TypeOf(onigenc_is_mbc_word_ascii(enc, s_2, end)) {
    return onigenc_is_mbc_word_ascii(enc, s_2, end);
}
pub fn ONIGENC_NAME(enc: anytype) callconv(.Inline) @TypeOf(enc.*.name) {
    return enc.*.name;
}
pub fn ONIGENC_IS_ALLOWED_REVERSE_MATCH(enc: anytype, s_2: anytype, end: anytype) callconv(.Inline) @TypeOf(enc.*.is_allowed_reverse_match(s_2, end)) {
    return enc.*.is_allowed_reverse_match(s_2, end);
}
pub fn ONIGENC_LEFT_ADJUST_CHAR_HEAD(enc: anytype, start: anytype, s_2: anytype) callconv(.Inline) @TypeOf(enc.*.left_adjust_char_head(start, s_2)) {
    return enc.*.left_adjust_char_head(start, s_2);
}
pub fn ONIGENC_IS_VALID_MBC_STRING(enc: anytype, s_2: anytype, end: anytype) callconv(.Inline) @TypeOf(enc.*.is_valid_mbc_string(s_2, end)) {
    return enc.*.is_valid_mbc_string(s_2, end);
}
...
pub fn ONIGENC_STEP_BACK(enc: anytype, start: anytype, s_2: anytype, n: anytype) callconv(.Inline) @TypeOf(onigenc_step_back(enc, start, s_2, n)) {
    return onigenc_step_back(enc, start, s_2, n);
}
...
pub fn __va_copy(d: anytype, s_2: anytype) callconv(.Inline) @TypeOf(__builtin_va_copy(d, s_2)) {
    return __builtin_va_copy(d, s_2);
}
...
pub fn __COPYRIGHT(s_2: anytype) callconv(.Inline) @TypeOf(__IDSTRING(copyright, s_2)) {
    return __IDSTRING(copyright, s_2);
}
pub fn __RCSID(s_2: anytype) callconv(.Inline) @TypeOf(__IDSTRING(rcsid, s_2)) {
    return __IDSTRING(rcsid, s_2);
}
pub fn __SCCSID(s_2: anytype) callconv(.Inline) @TypeOf(__IDSTRING(sccsid, s_2)) {
    return __IDSTRING(sccsid, s_2);
}
pub fn __PROJECT_VERSION(s_2: anytype) callconv(.Inline) @TypeOf(__IDSTRING(project_version, s_2)) {
    return __IDSTRING(project_version, s_2);
}
