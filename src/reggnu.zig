...
pub const RE_MBCTYPE_ASCII = @enumToInt(enum_unnamed_75.RE_MBCTYPE_ASCII);
pub const RE_MBCTYPE_EUC = @enumToInt(enum_unnamed_75.RE_MBCTYPE_EUC);
pub const RE_MBCTYPE_SJIS = @enumToInt(enum_unnamed_75.RE_MBCTYPE_SJIS);
pub const RE_MBCTYPE_UTF8 = @enumToInt(enum_unnamed_75.RE_MBCTYPE_UTF8);
...
pub fn re_mbcinit(arg_mb_code: c_int) callconv(.C) void {
...
pub fn re_compile_pattern(arg_pattern: [*c]const u8, arg_size: c_int, arg_reg: [*c]struct_re_pattern_buffer, arg_ebuf: [*c]u8) callconv(.C) c_int {
    ...
    r = onig_compile(reg, @intToPtr([*c]OnigUChar, @ptrToInt(pattern)), @intToPtr([*c]OnigUChar, @ptrToInt((pattern + size))), &einfo);
    if (r != @as(c_int, 0)) {
        if ((@ptrCast(?*c_void, (ebuf))) != @intToPtr(?*c_void, @as(c_int, 0))) _ = onig_error_code_to_str(@ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), ebuf)), r, &einfo);
    }
    return r;
}
pub extern fn re_recompile_pattern([*c]const u8, c_int, [*c]struct_re_pattern_buffer, err_buf: [*c]u8) c_int;
pub fn re_free_pattern(arg_reg: [*c]struct_re_pattern_buffer) callconv(.C) void {
    var reg = arg_reg;
    onig_free(reg);
}
pub fn re_adjust_startpos(arg_reg: [*c]struct_re_pattern_buffer, arg_string: [*c]const u8, arg_size: c_int, arg_startpos: c_int, arg_range: c_int) callconv(.C) c_int {
    var reg = arg_reg;
    var string = arg_string;
    var size = arg_size;
    var startpos = arg_startpos;
    var range = arg_range;
    if (((startpos > @as(c_int, 0)) and (((reg.*.enc).*.max_enc_len) != @as(c_int, 1))) and (startpos < size)) {
        var p: [*c]OnigUChar = undefined;
        var s: [*c]OnigUChar = (@intToPtr([*c]OnigUChar, @ptrToInt(string)) + startpos);
        if (range > @as(c_int, 0)) {
            p = onigenc_get_right_adjust_char_head(reg.*.enc, @intToPtr([*c]OnigUChar, @ptrToInt(string)), s);
        } else {
            p = (reg.*.enc).*.left_adjust_char_head.?(@intToPtr([*c]OnigUChar, @ptrToInt(string)), s);
        }
        return @bitCast(c_int, @truncate(c_int, (p - @intToPtr([*c]OnigUChar, @ptrToInt(string)))));
    }
    return startpos;
}
pub fn re_search(arg_bufp: [*c]struct_re_pattern_buffer, arg_string: [*c]const u8, arg_size: c_int, arg_startpos: c_int, arg_range: c_int, arg_regs: [*c]struct_re_registers) callconv(.C) c_int {
    var bufp = arg_bufp;
    var string = arg_string;
    var size = arg_size;
    var startpos = arg_startpos;
    var range = arg_range;
    var regs = arg_regs;
    return onig_search(bufp, @intToPtr([*c]OnigUChar, @ptrToInt(string)), @intToPtr([*c]OnigUChar, @ptrToInt((string + size))), @intToPtr([*c]OnigUChar, @ptrToInt((string + startpos))), @intToPtr([*c]OnigUChar, @ptrToInt(((string + startpos) + range))), regs, @as(c_uint, 0));
}
pub fn re_match(arg_reg: [*c]struct_re_pattern_buffer, arg_str: [*c]const u8, arg_size: c_int, arg_pos: c_int, arg_regs: [*c]struct_re_registers) callconv(.C) c_int {
    var reg = arg_reg;
    var str = arg_str;
    var size = arg_size;
    var pos = arg_pos;
    var regs = arg_regs;
    return onig_match(reg, @intToPtr([*c]OnigUChar, @ptrToInt(str)), @intToPtr([*c]OnigUChar, @ptrToInt((str + size))), @intToPtr([*c]OnigUChar, @ptrToInt((str + pos))), regs, @as(c_uint, 0));
}
pub fn re_set_casetable(arg_table: [*c]const u8) callconv(.C) void {
    var table = arg_table;
    onigenc_set_default_caseconv_table(@intToPtr([*c]OnigUChar, @ptrToInt(table)));
}
pub fn re_free_registers(arg_r: [*c]struct_re_registers) callconv(.C) void {
    var r = arg_r;
    onig_region_free(r, @as(c_int, 0));
}
pub fn re_alloc_pattern(arg_reg: [*c][*c]struct_re_pattern_buffer) callconv(.C) c_int {
    var reg = arg_reg;
    ...
    return onig_reg_init(reg.?.*, @as(c_uint, 0), OnigDefaultCaseFoldFlag, OnigEncDefaultCharEncoding, OnigDefaultSyntax);
}
...
pub const RE_NREGS = ONIG_NREGION;
pub const RE_OPTION_IGNORECASE = ONIG_OPTION_IGNORECASE;
pub const RE_OPTION_EXTENDED = ONIG_OPTION_EXTEND;
pub const RE_OPTION_MULTILINE = ONIG_OPTION_MULTILINE;
pub const RE_OPTION_SINGLELINE = ONIG_OPTION_SINGLELINE;
pub const RE_OPTION_LONGEST = ONIG_OPTION_FIND_LONGEST;
pub const RE_OPTION_POSIXLINE = RE_OPTION_MULTILINE | RE_OPTION_SINGLELINE;
pub const RE_OPTION_FIND_NOT_EMPTY = ONIG_OPTION_FIND_NOT_EMPTY;
pub const RE_OPTION_NEGATE_SINGLELINE = ONIG_OPTION_NEGATE_SINGLELINE;
pub const RE_OPTION_DONT_CAPTURE_GROUP = ONIG_OPTION_DONT_CAPTURE_GROUP;
pub const RE_OPTION_CAPTURE_GROUP = ONIG_OPTION_CAPTURE_GROUP;
