...
pub const EncISO_8859_9_ToLowerCaseTable: [256]OnigUChar = [256]OnigUChar{
...
pub const EncISO_8859_9_CtypeTable: [256]c_ushort = [256]c_ushort{
...
pub fn mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if ((((flag) & @bitCast(c_uint, (@as(c_int, 1)))) == @bitCast(c_uint, @as(c_int, 0))) or (@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 128))) lower.?.* = EncISO_8859_9_ToLowerCaseTable[p.?.*] else lower.?.* = p.?.*;
    (pp.?.*) += 1;
    return 1;
}
pub fn is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    if (code < @bitCast(c_uint, @as(c_int, 256))) return @boolToInt(((@bitCast(c_int, @as(c_uint, EncISO_8859_9_CtypeTable[code])) & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), (ctype)))) != @as(c_int, 0))) else return 0;
    return 0;
}
pub const CaseFoldMap: [30]OnigPairCaseFoldCodes = [30]OnigPairCaseFoldCodes{
...
pub fn apply_all_case_fold(arg_flag: OnigCaseFoldType, arg_f: OnigApplyAllCaseFoldFunc, arg_arg: ?*c_void) callconv(.C) c_int {
    ...
    var f = arg_f;
    var arg = arg_arg;
    return onigenc_apply_all_case_fold_with_map(@bitCast(c_int, @truncate(c_uint, (@sizeOf([30]OnigPairCaseFoldCodes) / @sizeOf(OnigPairCaseFoldCodes)))), &CaseFoldMap, @as(c_int, 1), flag, f, arg);
}
pub fn get_case_fold_codes_by_str(arg_flag: OnigCaseFoldType, arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_items: [*c]OnigCaseFoldCodeItem) callconv(.C) c_int {
    ...
    var end = arg_end;
    var items = arg_items;
    return onigenc_get_case_fold_codes_by_str_with_map(@bitCast(c_int, @truncate(c_uint, (@sizeOf([30]OnigPairCaseFoldCodes) / @sizeOf(OnigPairCaseFoldCodes)))), &CaseFoldMap, @as(c_int, 1), flag, p, end, items);
}
...
pub fn ENC_ISO_8859_9_TO_LOWER_CASE(c: anytype) callconv(.Inline) @TypeOf(EncISO_8859_9_ToLowerCaseTable[c]) {
    return EncISO_8859_9_ToLowerCaseTable[c];
}
pub fn ENC_IS_ISO_8859_9_CTYPE(code: anytype, ctype: anytype) callconv(.Inline) @TypeOf((EncISO_8859_9_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0) {
    return (EncISO_8859_9_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0;
}
