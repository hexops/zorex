...
pub fn utf32le_mbc_enc_len(arg_p: [*c]const OnigUChar) callconv(.C) c_int {
    var p = arg_p;
    return 4;
}
pub fn is_valid_mbc_string(arg_s: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    var s = arg_s;
    var end = arg_end;
    return onigenc_length_check_is_valid_mbc_string((&OnigEncodingUTF32_LE), s, end);
}
pub fn utf32le_is_mbc_newline(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    var p = arg_p;
    var end = arg_end;
    if ((p + @as(c_int, 3)) < end) {
        if ((((@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 10)) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 1)).?.*)) == @as(c_int, 0))) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 2)).?.*)) == @as(c_int, 0))) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 3)).?.*)) == @as(c_int, 0))) return 1;
    }
    return 0;
}
pub fn utf32le_mbc_to_code(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) OnigCodePoint {
    var p = arg_p;
    var end = arg_end;
    var code: OnigCodePoint = undefined;
    code = @bitCast(OnigCodePoint, (((((((@bitCast(c_int, @as(c_uint, p[@intCast(c_uint, @as(c_int, 3))])) & @as(c_int, 127)) * @as(c_int, 256)) + @bitCast(c_int, @as(c_uint, p[@intCast(c_uint, @as(c_int, 2))]))) * @as(c_int, 256)) + @bitCast(c_int, @as(c_uint, p[@intCast(c_uint, @as(c_int, 1))]))) * @as(c_int, 256)) + @bitCast(c_int, @as(c_uint, p[@intCast(c_uint, @as(c_int, 0))]))));
    return code;
}
pub fn utf32le_code_to_mbclen(arg_code: OnigCodePoint) callconv(.C) c_int {
    var code = arg_code;
    return 4;
}
pub fn utf32le_code_to_mbc(arg_code: OnigCodePoint, arg_buf: [*c]OnigUChar) callconv(.C) c_int {
    var code = arg_code;
    var buf = arg_buf;
    ...
    (blk: {
        ...
        ref.* += 1;
        break :blk tmp;
    }).?.* = @bitCast(OnigUChar, @truncate(u8, ((code & @bitCast(c_uint, @as(c_int, 16711680))) >> @intCast(@import("std").math.Log2Int(c_uint), 16))));
    (blk: {
        const ref = &p;
        const tmp = ref.*;
        ref.* += 1;
        break :blk tmp;
    }).?.* = @bitCast(OnigUChar, @truncate(u8, ((code & @as(c_uint, 4278190080)) >> @intCast(@import("std").math.Log2Int(c_uint), 24))));
    return 4;
}
pub fn utf32le_mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_fold: [*c]OnigUChar) callconv(.C) c_int {
    var flag = arg_flag;
    var pp = arg_pp;
    var end = arg_end;
    var fold = arg_fold;
    var p: [*c]const OnigUChar = pp.?.*;
    if ((((@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 128)) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 1)).?.*)) == @as(c_int, 0))) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 2)).?.*)) == @as(c_int, 0))) and (@bitCast(c_int, @as(c_uint, (p + @as(c_int, 3)).?.*)) == @as(c_int, 0))) {
        (blk: {
            const ref = &fold;
            const tmp = ref.*;
            ref.* += 1;
        ...
        pp.?.* += @as(c_int, 4);
        return 4;
    } else return onigenc_unicode_mbc_case_fold((&OnigEncodingUTF32_LE), flag, pp, end, fold);
    return 0;
}
pub fn utf32le_left_adjust_char_head(arg_start: [*c]const OnigUChar, arg_s: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var start = arg_start;
    var s = arg_s;
    var rem: c_int = undefined;
    if (s <= start) return @intToPtr([*c]OnigUChar, @ptrToInt(s));
    rem = @bitCast(c_int, @truncate(c_int, @rem((s - start), @bitCast(c_long, @as(c_long, @as(c_int, 4))))));
    return @intToPtr([*c]OnigUChar, @ptrToInt((s - rem)));
}
pub fn utf32le_get_case_fold_codes_by_str(arg_flag: OnigCaseFoldType, arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_items: [*c]OnigCaseFoldCodeItem) callconv(.C) c_int {
    var flag = arg_flag;
    var p = arg_p;
    var end = arg_end;
    var items = arg_items;
    return onigenc_unicode_get_case_fold_codes_by_str((&OnigEncodingUTF32_LE), flag, p, end, items);
}
