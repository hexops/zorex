...
pub const EncISO_8859_11_CtypeTable: [256]c_ushort = [256]c_ushort{
...
pub fn is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    if (code < @bitCast(c_uint, @as(c_int, 256))) return @boolToInt(((@bitCast(c_int, @as(c_uint, EncISO_8859_11_CtypeTable[code])) & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), (ctype)))) != @as(c_int, 0))) else return 0;
    return 0;
}
...
pub fn ENC_IS_ISO_8859_11_CTYPE(code: anytype, ctype: anytype) callconv(.Inline) @TypeOf((EncISO_8859_11_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0) {
    return (EncISO_8859_11_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0;
}
