...
pub const EncLen_EUCKR: [256]c_int = [256]c_int{
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
...
pub fn euckr_mbc_enc_len(arg_p: [*c]const OnigUChar) callconv(.C) c_int {
    var p = arg_p;
    return EncLen_EUCKR[p.?.*];
}
pub fn euckr_code_to_mbclen(arg_code: OnigCodePoint) callconv(.C) c_int {
    var code = arg_code;
    ...
    if ((code & @bitCast(c_uint, @as(c_int, 65280))) != @bitCast(c_uint, @as(c_int, 0))) {
        if (EncLen_EUCKR[@intCast(c_uint, (@bitCast(c_int, (code >> @intCast(@import("std").math.Log2Int(OnigCodePoint), 8))) & @as(c_int, 255)))] == @as(c_int, 2)) return 2;
    } else {
        if (EncLen_EUCKR[@intCast(c_uint, @bitCast(c_int, (code & @bitCast(c_uint, @as(c_int, 255)))))] == @as(c_int, 1)) return 1;
    }
    return -@as(c_int, 400);
}
...
pub fn euckr_mbc_to_code(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) OnigCodePoint {
    var p = arg_p;
    var end = arg_end;
    return onigenc_mbn_mbc_to_code((&OnigEncodingEUC_KR), p, end);
}
pub fn euckr_code_to_mbc(arg_code: OnigCodePoint, arg_buf: [*c]OnigUChar) callconv(.C) c_int {
    var code = arg_code;
    var buf = arg_buf;
    return onigenc_mb2_code_to_mbc((&OnigEncodingEUC_KR), code, buf);
}
pub fn euckr_mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    var flag = arg_flag;
    var pp = arg_pp;
    var end = arg_end;
    var lower = arg_lower;
    return onigenc_mbn_mbc_case_fold((&OnigEncodingEUC_KR), flag, pp, end, lower);
}
pub fn euckr_is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    return onigenc_mb2_is_code_ctype((&OnigEncodingEUC_KR), code, ctype);
}
pub fn euckr_left_adjust_char_head(arg_start: [*c]const OnigUChar, arg_s: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var start = arg_start;
    var s = arg_s;
    ...
    p = s;
    while (!((@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 161)) or (@bitCast(c_int, @as(c_uint, (p.?.*))) == @as(c_int, 255))) and (p > start)) p -= 1;
    len = (&OnigEncodingEUC_KR).*.mbc_enc_len.?(p);
...
pub fn euckr_is_allowed_reverse_match(arg_s: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    var s = arg_s;
    var end = arg_end;
    const c: OnigUChar = s.?.*;
...
pub fn euckr_islead(c: anytype) callconv(.Inline) @TypeOf((c < 0xa1) or (c == 0xff)) {
    return (c < 0xa1) or (c == 0xff);
}
