...
pub const EncISO_8859_1_CtypeTable: [256]c_ushort = [256]c_ushort{
...
} // ../oniguruma/src/iso8859_1.c:128:5: warning: TODO implement translation of stmt class LabelStmtClass
: varend =arg_end;arg_end : varlower=arg_lower; *c]const OnigUChar=pp. arg_lower:varp :[?.*; .*@bitCast(c_int ,@as(c_uint if((p .?,) ==@as
pub const get_case_fold_codes_by_str = @compileError("unable to translate function"); // ../oniguruma/src/iso8859_1.c:117:1
pub fn mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    ...
    if ((((flag) & @bitCast(c_uint, (@as(c_int, 1)))) == @bitCast(c_uint, @as(c_int, 0))) or (@bitCast(c_int, @as(c_uint, (p.?.*))) < @as(c_int, 128))) lower.?.* = OnigEncISO_8859_1_ToLowerCaseTable[p.?.*] else lower.?.* = p.?.*;
    (pp.?.*) += 1;
    return 1;
}
pub fn is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    if (code < @bitCast(c_uint, @as(c_int, 256))) return @boolToInt(((@bitCast(c_int, @as(c_uint, EncISO_8859_1_CtypeTable[code])) & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), (ctype)))) != @as(c_int, 0))) else return 0;
    return 0;
}
...
pub fn ENC_IS_ISO_8859_1_CTYPE(code: anytype, ctype: anytype) callconv(.Inline) @TypeOf((EncISO_8859_1_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0) {
    return (EncISO_8859_1_CtypeTable[code] & CTYPE_TO_BIT(ctype)) != 0;
}
