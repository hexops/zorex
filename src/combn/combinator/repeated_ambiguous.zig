...
pub fn RepeatedAmbiguous(comptime Payload: type, comptime Value: type) type {
    return struct {
        ...
        pub fn parse(parser: *const Parser(Payload, RepeatedAmbiguousValue(Value)), in_ctx: *const ParserContext(Payload, RepeatedAmbiguousValue(Value))) callconv(.Async) Error!void {
            const self = @fieldParentPtr(Self, "parser", parser);
...
test "repeated" {
    nosuspend {
        ...
        const Payload = void;
        const ctx = try ParserContext(Payload, RepeatedAmbiguousValue(LiteralValue)).init(allocator, "abcabcabc123abc", {});
        defer ctx.deinit();
