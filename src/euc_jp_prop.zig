...
var wordlist: [56]struct_PropertyNameCtype = [56]struct_PropertyNameCtype{
    struct_PropertyNameCtype{
        .name = "",
        .ctype = 0,
    },
    struct_PropertyNameCtype{
...
pub export fn onigenc_euc_jp_lookup_property_name(arg_str: [*c]const u8, arg_len: usize) [*c]struct_PropertyNameCtype {
    var str = arg_str;
    var len = arg_len;
    _ = wordlist;
    if ((len <= @bitCast(c_ulong, @as(c_long, @as(c_int, 8)))) and (len >= @bitCast(c_ulong, @as(c_long, @as(c_int, 4))))) {
        var key: c_uint = hash(str, len);
        if (key <= @bitCast(c_uint, @as(c_int, 55))) {
            var s: [*c]const u8 = wordlist[key].name;
            if ((@bitCast(c_int, @as(c_uint, str.?.*)) == @bitCast(c_int, @as(c_uint, s.?.*))) and !(strcmp((str + @as(c_int, 1)), (s + @as(c_int, 1))) != 0)) return &wordlist[key];
        }
    }
    return null;
}
...
pub fn hash(arg_str: [*c]const u8, arg_len: usize) callconv(.C) c_uint {
    var str = arg_str;
    var len = arg_len;
    _ = asso_values;
    return ((@bitCast(c_uint, @truncate(c_uint, len)) +% @bitCast(c_uint, @as(c_uint, asso_values[@bitCast(u8, str[@intCast(c_uint, @as(c_int, 2))])]))) +% @bitCast(c_uint, @as(c_uint, asso_values[@bitCast(u8, str[@intCast(c_uint, @as(c_int, 0))])])));
}
...
pub const TOTAL_KEYWORDS = 16;
pub const MIN_WORD_LENGTH = 4;
pub const MAX_WORD_LENGTH = 8;
pub const MIN_HASH_VALUE = 4;
pub const MAX_HASH_VALUE = 55;
