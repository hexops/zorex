...
pub const struct_OnigRegSetStruct = extern struct {
    rs: [*c]RR,
    n: c_int,
    alloc: c_int,
    enc: OnigEncoding,
...
pub const struct_OnigCalloutArgsStruct = extern struct {
    in: OnigCalloutIn,
    name_id: c_int,
    num: c_int,
    regex: OnigRegex,
    string: [*c]const OnigUChar,
    string_end: [*c]const OnigUChar,
    start: [*c]const OnigUChar,
    right_range: [*c]const OnigUChar,
    current: [*c]const OnigUChar,
    retry_in_match_counter: c_ulong,
    msa: [*c]MatchArg,
    stk_base: [*c]StackType,
    stk: [*c]StackType,
    mem_start_stk: [*c]StkPtrType,
    mem_end_stk: [*c]StkPtrType,
};
pub const OnigCalloutArgs = struct_OnigCalloutArgsStruct;
pub const OnigCalloutFunc = ?fn ([*c]OnigCalloutArgs, ?*c_void) callconv(.C) c_int;
...
pub const struct_OnigMatchParamStruct = extern struct {
    match_stack_limit: c_uint,
    retry_limit_in_match: c_ulong,
    retry_limit_in_search: c_ulong,
    progress_callout_of_contents: OnigCalloutFunc,
    retraction_callout_of_contents: OnigCalloutFunc,
    match_at_call_counter: c_int,
    callout_user_data: ?*c_void,
    callout_data: [*c]CalloutData,
    callout_data_alloc_num: c_int,
};
...
pub fn onig_scan(arg_reg: OnigRegex, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_region: [*c]OnigRegion, arg_option: OnigOptionType, arg_scan_callback: ?fn (c_int, c_int, [*c]OnigRegion, ?*c_void) callconv(.C) c_int, arg_callback_arg: ?*c_void) callconv(.C) c_int {
    var reg = arg_reg;
    var str = arg_str;
    var end = arg_end;
    var region = arg_region;
    var option = arg_option;
    var scan_callback = arg_scan_callback;
    var callback_arg = arg_callback_arg;
    var r: c_int = undefined;
    var n: c_int = undefined;
    var rs: c_int = undefined;
    var start: [*c]const OnigUChar = undefined;
    if (((option) & ((((((((((((@as(c_uint, 1) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1))) != 0) {
        if (!((reg.*.enc).*.is_valid_mbc_string.?(str, end) != 0)) return -@as(c_int, 400);
        _ = (blk: {
            const ref = &(option);
            ref.* = ref.* & ~((((((((((((@as(c_uint, 1) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1));
            break :blk ref.*;
        });
    }
    n = 0;
    ...
    while (true) {
        ...
        if (r >= @as(c_int, 0)) {
            ...
            n += 1;
            if (rs != @as(c_int, 0)) return rs;
            if (@bitCast(c_long, @as(c_long, region.*.end[@intCast(c_uint, @as(c_int, 0))])) == (start - str)) {
                if (start >= end) break;
                start += (reg.*.enc).*.mbc_enc_len.?(start);
            } else start = (str + region.*.end[@intCast(c_uint, @as(c_int, 0))]);
            if (start > end) break;
        } else if (r == -@as(c_int, 1)) {
            break;
        } else {
            return r;
        }
...
pub fn onig_search(arg_reg: OnigRegex, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_start: [*c]const OnigUChar, arg_range: [*c]const OnigUChar, arg_region: [*c]OnigRegion, arg_option: OnigOptionType) callconv(.C) c_int {
    var reg = arg_reg;
    var str = arg_str;
    var end = arg_end;
    var start = arg_start;
    ...
    var option = arg_option;
    var r: c_int = undefined;
    var mp: OnigMatchParam = undefined;
    var data_range: [*c]const OnigUChar = undefined;
    _ = onig_initialize_match_param(&mp);
    if (range > start) data_range = range else data_range = end;
    r = search_in_range(reg, str, end, start, range, data_range, region, option, &mp);
    onig_free_match_param_content(&mp);
    return r;
}
pub fn onig_search_with_param(arg_reg: OnigRegex, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_start: [*c]const OnigUChar, arg_range: [*c]const OnigUChar, arg_region: [*c]OnigRegion, arg_option: OnigOptionType, arg_mp: [*c]OnigMatchParam) callconv(.C) c_int {
    var reg = arg_reg;
    var str = arg_str;
    var end = arg_end;
    var start = arg_start;
...
pub fn onig_match(arg_reg: OnigRegex, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_at: [*c]const OnigUChar, arg_region: [*c]OnigRegion, arg_option: OnigOptionType) callconv(.C) c_int {
    var reg = arg_reg;
    var str = arg_str;
    var end = arg_end;
    var at = arg_at;
    ...
    onig_free_match_param_content(&mp);
    return r;
} // ../oniguruma/src/regexec.c:5090:9: warning: TODO implement translation of stmt class GotoStmtClass
pub const onig_match_with_param = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:5063:1
pub fn onig_regset_new(arg_rset: [*c][*c]OnigRegSet, arg_n: c_int, arg_regs: [*c][*c]regex_t) callconv(.C) c_int {
    var rset = arg_rset;
    var n = arg_n;
    var regs = arg_regs;
    ...
    var set: [*c]OnigRegSet = undefined;
    var rs: [*c]RR = undefined;
    rset.?.* = null;
    set = @ptrCast([*c]OnigRegSet, @alignCast(@alignOf(OnigRegSet), malloc(@sizeOf(OnigRegSet))));
    if ((@ptrCast(?*c_void, (set))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
    alloc = if (n > @as(c_int, 10)) n else @as(c_int, 10);
    rs = @ptrCast([*c]RR, @alignCast(@alignOf(RR), malloc((@sizeOf(RR) *% @bitCast(c_ulong, @as(c_long, alloc))))));
    if ((@ptrCast(?*c_void, (rs))) == @intToPtr(?*c_void, @as(c_int, 0))) {
        free(@ptrCast(?*c_void, set));
        return -@as(c_int, 5);
    }
    set.*.rs = rs;
    set.*.n = 0;
    set.*.alloc = alloc;
    {
        i = 0;
        while (i < n) : (i += 1) {
            var reg: [*c]regex_t = regs[@intCast(c_uint, i)];
            r = onig_regset_add(set, reg);
            if (r != @as(c_int, 0)) {
                {
                    i = 0;
                    while (i < set.*.n) : (i += 1) {
                        var region: [*c]OnigRegion = set.*.rs[@intCast(c_uint, i)].region;
                        if ((@ptrCast(?*c_void, (region))) != @intToPtr(?*c_void, @as(c_int, 0))) onig_region_free(region, @as(c_int, 1));
                    }
                }
                free(@ptrCast(?*c_void, set.*.rs));
                free(@ptrCast(?*c_void, set));
                return r;
            }
        }
...
pub fn onig_regset_add(arg_set: [*c]OnigRegSet, arg_reg: [*c]regex_t) callconv(.C) c_int {
    var set = arg_set;
    var reg = arg_reg;
    var region: [*c]OnigRegion = undefined;
    if (((reg.*.options) & ((((@as(c_uint, 1) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1))) != 0) return -@as(c_int, 30);
    if ((set.*.n != @as(c_int, 0)) and (reg.*.enc != set.*.enc)) return -@as(c_int, 30);
    if (set.*.n >= set.*.alloc) {
        var nrs: [*c]RR = undefined;
        var new_alloc: c_int = undefined;
        new_alloc = (set.*.alloc * @as(c_int, 2));
        nrs = @ptrCast([*c]RR, @alignCast(@alignOf(RR), realloc(@ptrCast(?*c_void, set.*.rs), (@sizeOf(RR) *% @bitCast(c_ulong, @as(c_long, new_alloc))))));
        if ((@ptrCast(?*c_void, (nrs))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
        set.*.rs = nrs;
        set.*.alloc = new_alloc;
    }
    region = onig_region_new();
    if ((@ptrCast(?*c_void, (region))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
    set.*.rs[@intCast(c_uint, set.*.n)].reg = reg;
    set.*.rs[@intCast(c_uint, set.*.n)].region = region;
    set.*.n += 1;
    update_regset_by_reg(set, reg);
    return 0;
}
pub fn onig_regset_replace(arg_set: [*c]OnigRegSet, arg_at: c_int, arg_reg: [*c]regex_t) callconv(.C) c_int {
    var set = arg_set;
    var at = arg_at;
    var reg = arg_reg;
    var i: c_int = undefined;
    if ((at < @as(c_int, 0)) or (at >= set.*.n)) return -@as(c_int, 30);
    if ((@ptrCast(?*c_void, (reg))) == @intToPtr(?*c_void, @as(c_int, 0))) {
        onig_region_free(set.*.rs[@intCast(c_uint, at)].region, @as(c_int, 1));
        {
            i = at;
            while (i < (set.*.n - @as(c_int, 1))) : (i += 1) {
                set.*.rs[@intCast(c_uint, i)].reg = set.*.rs[@intCast(c_uint, (i + @as(c_int, 1)))].reg;
                set.*.rs[@intCast(c_uint, i)].region = set.*.rs[@intCast(c_uint, (i + @as(c_int, 1)))].region;
            }
        }
        set.*.n -= 1;
    } else {
        if (((reg.*.options) & ((((@as(c_uint, 1) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1)) << @intCast(@import("std").math.Log2Int(c_uint), 1))) != 0) return -@as(c_int, 30);
        if ((set.*.n > @as(c_int, 1)) and (reg.*.enc != set.*.enc)) return -@as(c_int, 30);
        set.*.rs[@intCast(c_uint, at)].reg = reg;
    }
    {
        i = 0;
        while (i < set.*.n) : (i += 1) update_regset_by_reg(set, set.*.rs[@intCast(c_uint, i)].reg);
    }
    return 0;
}
pub fn onig_regset_free(arg_set: [*c]OnigRegSet) callconv(.C) void {
    var set = arg_set;
    var i: c_int = undefined;
    {
        i = 0;
        while (i < set.*.n) : (i += 1) {
            ...
            var region: [*c]OnigRegion = undefined;
            reg = set.*.rs[@intCast(c_uint, i)].reg;
            region = set.*.rs[@intCast(c_uint, i)].region;
            onig_free(reg);
            if ((@ptrCast(?*c_void, (region))) != @intToPtr(?*c_void, @as(c_int, 0))) onig_region_free(region, @as(c_int, 1));
        }
    }
...
pub fn onig_regset_number_of_regex(arg_set: [*c]OnigRegSet) callconv(.C) c_int {
    var set = arg_set;
    return set.*.n;
}
pub fn onig_regset_get_regex(arg_set: [*c]OnigRegSet, arg_at: c_int) callconv(.C) [*c]regex_t {
    var set = arg_set;
    var at = arg_at;
    if ((at < @as(c_int, 0)) or (at >= set.*.n)) return @intToPtr([*c]regex_t, @as(c_int, 0));
    return set.*.rs[@intCast(c_uint, at)].reg;
}
pub fn onig_regset_get_region(arg_set: [*c]OnigRegSet, arg_at: c_int) callconv(.C) [*c]OnigRegion {
    var set = arg_set;
    var at = arg_at;
    if ((at < @as(c_int, 0)) or (at >= set.*.n)) return @intToPtr([*c]OnigRegion, @as(c_int, 0));
    return set.*.rs[@intCast(c_uint, at)].region;
}
pub fn onig_regset_search(arg_set: [*c]OnigRegSet, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_start: [*c]const OnigUChar, arg_range: [*c]const OnigUChar, arg_lead: OnigRegSetLead, arg_option: OnigOptionType, arg_rmatch_pos: [*c]c_int) callconv(.C) c_int {
    var set = arg_set;
    var str = arg_str;
    var end = arg_end;
    var start = arg_start;
    ...
    var lead = arg_lead;
    var option = arg_option;
    var rmatch_pos = arg_rmatch_pos;
    var r: c_int = undefined;
    var i: c_int = undefined;
    var mp: [*c]OnigMatchParam = undefined;
    var mps: [*c][*c]OnigMatchParam = undefined;
    mps = @ptrCast([*c][*c]OnigMatchParam, @alignCast(@alignOf([*c]OnigMatchParam), malloc(((@sizeOf([*c]OnigMatchParam) +% @sizeOf(OnigMatchParam)) *% @bitCast(c_ulong, @as(c_long, set.*.n))))));
    if ((@ptrCast(?*c_void, (mps))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
    mp = @ptrCast([*c]OnigMatchParam, @alignCast(@alignOf(OnigMatchParam), (mps + set.*.n)));
    {
        i = 0;
        while (i < set.*.n) : (i += 1) {
            _ = onig_initialize_match_param((mp + i));
            mps[@intCast(c_uint, i)] = (mp + i);
        }
    }
    r = onig_regset_search_with_param(set, str, end, start, range, lead, option, mps, rmatch_pos);
    {
        i = 0;
        while (i < set.*.n) : (i += 1) onig_free_match_param_content((mp + i));
    }
    free(@ptrCast(?*c_void, mps));
    return r;
} // ../oniguruma/src/regexec.c:4656:19: warning: TODO implement translation of stmt class GotoStmtClass
pub const onig_regset_search_with_param = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:4624:1
pub fn onig_region_new() callconv(.C) [*c]OnigRegion {
    var r: [*c]OnigRegion = undefined;
    r = @ptrCast([*c]OnigRegion, @alignCast(@alignOf(OnigRegion), malloc(@sizeOf(OnigRegion))));
    if ((@ptrCast(?*c_void, (r))) == @intToPtr(?*c_void, @as(c_int, 0))) return null;
    onig_region_init(r);
    return r;
}
pub fn onig_region_init(arg_region: [*c]OnigRegion) callconv(.C) void {
    var region = arg_region;
    region.*.num_regs = 0;
    region.*.allocated = 0;
    region.*.beg = @intToPtr([*c]c_int, @as(c_int, 0));
    region.*.end = @intToPtr([*c]c_int, @as(c_int, 0));
    region.*.history_root = @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0));
}
pub fn onig_region_free(arg_r: [*c]OnigRegion, arg_free_self: c_int) callconv(.C) void {
    var r = arg_r;
    var free_self = arg_free_self;
    if (r != null) {
        if (r.*.allocated > @as(c_int, 0)) {
            if (r.*.beg != null) free(@ptrCast(?*c_void, r.*.beg));
            if (r.*.end != null) free(@ptrCast(?*c_void, r.*.end));
            r.*.allocated = 0;
        }
        history_root_free(r);
        if (free_self != 0) free(@ptrCast(?*c_void, r));
    }
}
pub fn onig_region_copy(arg_to: [*c]OnigRegion, arg_from: [*c]OnigRegion) callconv(.C) void {
    var to = arg_to;
    var from = arg_from;
    var i: c_int = undefined;
    if (to == from) return;
    if (to.*.allocated == @as(c_int, 0)) {
        if (from.*.num_regs > @as(c_int, 0)) {
            to.*.beg = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), malloc((@sizeOf(c_int) *% @bitCast(c_ulong, @as(c_long, from.*.num_regs))))));
            if ((@ptrCast(?*c_void, (to.*.beg))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
            to.*.end = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), malloc((@sizeOf(c_int) *% @bitCast(c_ulong, @as(c_long, from.*.num_regs))))));
            if ((@ptrCast(?*c_void, (to.*.end))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
            to.*.allocated = from.*.num_regs;
        }
    } else if (to.*.allocated < from.*.num_regs) {
        to.*.beg = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), realloc(@ptrCast(?*c_void, to.*.beg), (@sizeOf(c_int) *% @bitCast(c_ulong, @as(c_long, from.*.num_regs))))));
        if ((@ptrCast(?*c_void, (to.*.beg))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
        to.*.end = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), realloc(@ptrCast(?*c_void, to.*.end), (@sizeOf(c_int) *% @bitCast(c_ulong, @as(c_long, from.*.num_regs))))));
        if ((@ptrCast(?*c_void, (to.*.end))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
        to.*.allocated = from.*.num_regs;
    }
    {
        i = 0;
        while (i < from.*.num_regs) : (i += 1) {
            to.*.beg[@intCast(c_uint, i)] = from.*.beg[@intCast(c_uint, i)];
            to.*.end[@intCast(c_uint, i)] = from.*.end[@intCast(c_uint, i)];
        }
    }
    to.*.num_regs = from.*.num_regs;
    history_root_free(to);
    if ((@ptrCast(?*c_void, (from.*.history_root))) != @intToPtr(?*c_void, @as(c_int, 0))) {
        to.*.history_root = history_tree_clone(from.*.history_root);
    }
}
pub fn onig_region_clear(arg_region: [*c]OnigRegion) callconv(.C) void {
    var region = arg_region;
    var i: c_int = undefined;
    {
        i = 0;
        while (i < region.*.num_regs) : (i += 1) {
            region.*.beg[@intCast(c_uint, i)] = blk: {
                const tmp = -@as(c_int, 1);
                region.*.end[@intCast(c_uint, i)] = tmp;
                break :blk tmp;
            };
        }
    }
...
pub fn onig_region_resize(arg_region: [*c]OnigRegion, arg_n: c_int) callconv(.C) c_int {
    var region = arg_region;
    var n = arg_n;
    region.*.num_regs = n;
    if (n < @as(c_int, 10)) n = 10;
    if (region.*.allocated == @as(c_int, 0)) {
        region.*.beg = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), malloc((@bitCast(c_ulong, @as(c_long, n)) *% @sizeOf(c_int)))));
        region.*.end = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), malloc((@bitCast(c_ulong, @as(c_long, n)) *% @sizeOf(c_int)))));
        if ((region.*.beg == null) or (region.*.end == null)) return -@as(c_int, 5);
        region.*.allocated = n;
    } else if (region.*.allocated < n) {
        region.*.beg = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), realloc(@ptrCast(?*c_void, region.*.beg), (@bitCast(c_ulong, @as(c_long, n)) *% @sizeOf(c_int)))));
        region.*.end = @ptrCast([*c]c_int, @alignCast(@alignOf(c_int), realloc(@ptrCast(?*c_void, region.*.end), (@bitCast(c_ulong, @as(c_long, n)) *% @sizeOf(c_int)))));
        if ((region.*.beg == null) or (region.*.end == null)) return -@as(c_int, 5);
        region.*.allocated = n;
    }
    return 0;
}
pub fn onig_region_set(arg_region: [*c]OnigRegion, arg_at: c_int, arg_beg: c_int, arg_end: c_int) callconv(.C) c_int {
    var region = arg_region;
    var at = arg_at;
    var beg = arg_beg;
    var end = arg_end;
    if (at < @as(c_int, 0)) return -@as(c_int, 30);
    if (at >= region.*.allocated) {
        var r: c_int = onig_region_resize(region, (at + @as(c_int, 1)));
        if (r < @as(c_int, 0)) return r;
    }
    region.*.beg[@intCast(c_uint, at)] = beg;
    region.*.end[@intCast(c_uint, at)] = end;
    return 0;
}
...
pub fn onig_number_of_captures(arg_reg: OnigRegex) callconv(.C) c_int {
    var reg = arg_reg;
    return reg.*.num_mem;
}
pub fn onig_number_of_capture_histories(arg_reg: OnigRegex) callconv(.C) c_int {
    var reg = arg_reg;
    var i: c_int = undefined;
    var n: c_int = undefined;
    n = 0;
    {
        ...
        while (i <= @as(c_int, 31)) : (i += 1) {
            if ((if ((i) < @bitCast(c_int, @truncate(c_uint, (@sizeOf(MemStatusType) *% @bitCast(c_ulong, @as(c_long, @as(c_int, 8))))))) ((reg.*.capture_history) & (@bitCast(MemStatusType, @as(c_int, 1)) << @intCast(@import("std").math.Log2Int(MemStatusType), i))) else ((reg.*.capture_history) & @bitCast(c_uint, @as(c_int, 1)))) != @bitCast(c_uint, @as(c_int, 0))) n += 1;
        }
    }
    return n;
}
pub fn onig_get_capture_tree(arg_region: [*c]OnigRegion) callconv(.C) [*c]OnigCaptureTreeNode {
    var region = arg_region;
...
pub fn onig_get_encoding(arg_reg: OnigRegex) callconv(.C) OnigEncoding {
    var reg = arg_reg;
    return reg.*.enc;
}
pub fn onig_get_options(arg_reg: OnigRegex) callconv(.C) OnigOptionType {
    var reg = arg_reg;
    return reg.*.options;
}
pub fn onig_get_case_fold_flag(arg_reg: OnigRegex) callconv(.C) OnigCaseFoldType {
    var reg = arg_reg;
    return reg.*.case_fold_flag;
}
pub fn onig_get_syntax(arg_reg: OnigRegex) callconv(.C) [*c]OnigSyntaxType {
    var reg = arg_reg;
    return reg.*.syntax;
}
...
pub fn onig_copy_encoding(arg_to: OnigEncoding, arg_from: OnigEncoding) callconv(.C) void {
    var to = arg_to;
    var from = arg_from;
    to.?.* = from.?.*;
}
...
pub fn onig_get_match_stack_limit_size() callconv(.C) c_uint {
    return MatchStackLimit;
}
pub fn onig_set_match_stack_limit_size(arg_size: c_uint) callconv(.C) c_int {
    var size = arg_size;
    MatchStackLimit = size;
    return 0;
}
pub fn onig_get_retry_limit_in_match() callconv(.C) c_ulong {
    return RetryLimitInMatch;
}
pub fn onig_set_retry_limit_in_match(arg_n: c_ulong) callconv(.C) c_int {
    var n = arg_n;
    RetryLimitInMatch = n;
    return 0;
}
pub fn onig_get_retry_limit_in_search() callconv(.C) c_ulong {
    return RetryLimitInSearch;
}
pub fn onig_set_retry_limit_in_search(arg_n: c_ulong) callconv(.C) c_int {
    var n = arg_n;
...
pub fn onig_get_subexp_call_limit_in_search() callconv(.C) c_ulong {
    return SubexpCallLimitInSearch;
}
pub fn onig_set_subexp_call_limit_in_search(arg_n: c_ulong) callconv(.C) c_int {
    var n = arg_n;
    SubexpCallLimitInSearch = n;
    return 0;
}
pub fn onig_get_subexp_call_max_nest_level() callconv(.C) c_int {
    return SubexpCallMaxNestLevel;
}
pub fn onig_set_subexp_call_max_nest_level(arg_level: c_int) callconv(.C) c_int {
    var level = arg_level;
...
pub fn onig_new_match_param() callconv(.C) [*c]OnigMatchParam {
    var p: [*c]OnigMatchParam = undefined;
    p = @ptrCast([*c]OnigMatchParam, @alignCast(@alignOf(OnigMatchParam), malloc(@sizeOf(OnigMatchParam))));
    if ((@ptrCast(?*c_void, (p))) != @intToPtr(?*c_void, @as(c_int, 0))) {
        _ = onig_initialize_match_param(p);
    }
    return p;
}
pub fn onig_free_match_param(arg_p: [*c]OnigMatchParam) callconv(.C) void {
    var p = arg_p;
    if ((@ptrCast(?*c_void, (p))) != @intToPtr(?*c_void, @as(c_int, 0))) {
        onig_free_match_param_content(p);
        free(@ptrCast(?*c_void, p));
    }
}
pub fn onig_free_match_param_content(arg_p: [*c]OnigMatchParam) callconv(.C) void {
    var p = arg_p;
    if ((@ptrCast(?*c_void, (p.*.callout_data))) != @intToPtr(?*c_void, @as(c_int, 0))) {
        free(@ptrCast(?*c_void, p.*.callout_data));
        p.*.callout_data = null;
    }
}
pub fn onig_initialize_match_param(arg_mp: [*c]OnigMatchParam) callconv(.C) c_int {
    var mp = arg_mp;
    ...
    mp.*.callout_user_data = null;
    mp.*.callout_data = null;
    mp.*.callout_data_alloc_num = 0;
    return 0;
}
pub fn onig_set_match_stack_limit_size_of_match_param(arg_param: [*c]OnigMatchParam, arg_limit: c_uint) callconv(.C) c_int {
    var param = arg_param;
    var limit = arg_limit;
    param.*.match_stack_limit = limit;
    return 0;
}
pub fn onig_set_retry_limit_in_match_of_match_param(arg_param: [*c]OnigMatchParam, arg_limit: c_ulong) callconv(.C) c_int {
    var param = arg_param;
    var limit = arg_limit;
    param.*.retry_limit_in_match = limit;
    return 0;
}
pub fn onig_set_retry_limit_in_search_of_match_param(arg_param: [*c]OnigMatchParam, arg_limit: c_ulong) callconv(.C) c_int {
    var param = arg_param;
    var limit = arg_limit;
    param.*.retry_limit_in_search = limit;
    return 0;
}
pub fn onig_set_progress_callout_of_match_param(arg_param: [*c]OnigMatchParam, arg_f: OnigCalloutFunc) callconv(.C) c_int {
    var param = arg_param;
    var f = arg_f;
    param.*.progress_callout_of_contents = f;
    return 0;
}
pub fn onig_set_retraction_callout_of_match_param(arg_param: [*c]OnigMatchParam, arg_f: OnigCalloutFunc) callconv(.C) c_int {
    var param = arg_param;
    var f = arg_f;
    param.*.retraction_callout_of_contents = f;
    return 0;
}
pub fn onig_set_callout_user_data_of_match_param(arg_param: [*c]OnigMatchParam, arg_user_data: ?*c_void) callconv(.C) c_int {
    var param = arg_param;
    var user_data = arg_user_data;
    param.*.callout_user_data = user_data;
    return 0;
}
pub fn onig_get_progress_callout() callconv(.C) OnigCalloutFunc {
    return DefaultProgressCallout;
}
pub fn onig_set_progress_callout(arg_f: OnigCalloutFunc) callconv(.C) c_int {
    var f = arg_f;
    DefaultProgressCallout = f;
    return 0;
}
pub fn onig_get_retraction_callout() callconv(.C) OnigCalloutFunc {
    return DefaultRetractionCallout;
}
pub fn onig_set_retraction_callout(arg_f: OnigCalloutFunc) callconv(.C) c_int {
    var f = arg_f;
...
pub fn onig_get_callout_data_by_tag(arg_reg: OnigRegex, arg_mp: [*c]OnigMatchParam, arg_tag: [*c]const OnigUChar, arg_tag_end: [*c]const OnigUChar, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    var tag = arg_tag;
    ...
    if (num == @as(c_int, 0)) return -@as(c_int, 231);
    return onig_get_callout_data(reg, mp, num, slot, type_1, val);
}
pub fn onig_set_callout_data_by_tag(arg_reg: OnigRegex, arg_mp: [*c]OnigMatchParam, arg_tag: [*c]const OnigUChar, arg_tag_end: [*c]const OnigUChar, arg_slot: c_int, arg_type_1: OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    var tag = arg_tag;
    var tag_end = arg_tag_end;
    ...
    if (num < @as(c_int, 0)) return num;
    if (num == @as(c_int, 0)) return -@as(c_int, 231);
    return onig_set_callout_data(reg, mp, num, slot, type_1, val);
}
pub fn onig_get_callout_num_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_int {
    var args = arg_args;
    return args.*.num;
}
pub fn onig_get_callout_in_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) OnigCalloutIn {
    var args = arg_args;
    return args.*.in;
}
pub fn onig_get_name_id_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_int {
    var args = arg_args;
    return args.*.name_id;
}
pub fn onig_get_contents_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    var num: c_int = undefined;
    ...
    if (@bitCast(c_uint, @enumToInt(e.*.of)) == @bitCast(c_uint, ONIG_CALLOUT_OF_CONTENTS)) {
        return e.*.u.content.start;
    }
    return null;
}
pub fn onig_get_contents_end_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    var num: c_int = undefined;
    var e: [*c]CalloutListEntry = undefined;
    num = args.*.num;
...
pub fn onig_get_args_num_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_int {
    var args = arg_args;
    var num: c_int = undefined;
    var e: [*c]CalloutListEntry = undefined;
    num = args.*.num;
    e = onig_reg_callout_list_at(args.*.regex, num);
    if ((@ptrCast(?*c_void, (e))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 30);
    if (@bitCast(c_uint, @enumToInt(e.*.of)) == @bitCast(c_uint, ONIG_CALLOUT_OF_NAME)) {
        return e.*.u.arg.num;
    }
    return -@as(c_int, 30);
}
pub fn onig_get_passed_args_num_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_int {
    var args = arg_args;
    var num: c_int = undefined;
    var e: [*c]CalloutListEntry = undefined;
    num = args.*.num;
...
pub fn onig_get_arg_by_callout_args(arg_args: [*c]OnigCalloutArgs, arg_index_1: c_int, arg_type_2: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var index_1 = arg_index_1;
    var type_2 = arg_type_2;
    var val = arg_val;
    var num: c_int = undefined;
    var e: [*c]CalloutListEntry = undefined;
    num = args.*.num;
    ...
    if ((@ptrCast(?*c_void, (e))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 30);
    if (@bitCast(c_uint, @enumToInt(e.*.of)) == @bitCast(c_uint, ONIG_CALLOUT_OF_NAME)) {
        if ((@ptrCast(?*c_void, (type_2))) != @intToPtr(?*c_void, @as(c_int, 0))) type_2.?.* = e.*.u.arg.types[@intCast(c_uint, index_1)];
        if ((@ptrCast(?*c_void, (val))) != @intToPtr(?*c_void, @as(c_int, 0))) val.?.* = e.*.u.arg.vals[@intCast(c_uint, index_1)];
        return 0;
    }
    return -@as(c_int, 30);
}
pub fn onig_get_string_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    return args.*.string;
}
pub fn onig_get_string_end_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    return args.*.string_end;
}
pub fn onig_get_start_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    return args.*.start;
}
pub fn onig_get_right_range_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    return args.*.right_range;
}
pub fn onig_get_current_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) [*c]const OnigUChar {
    var args = arg_args;
    return args.*.current;
}
pub fn onig_get_regex_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) OnigRegex {
    var args = arg_args;
    return args.*.regex;
}
pub fn onig_get_retry_counter_by_callout_args(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_ulong {
    var args = arg_args;
...
pub fn onig_get_callout_data_dont_clear_old(arg_reg: OnigRegex, arg_mp: [*c]OnigMatchParam, arg_callout_num: c_int, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    ...
    var type_1 = arg_type_1;
    var val = arg_val;
    var t: OnigType = undefined;
    var d: [*c]CalloutData = undefined;
    if (callout_num <= @as(c_int, 0)) return -@as(c_int, 30);
    d = ((mp).*.callout_data + ((callout_num) - @as(c_int, 1)));
    t = d.*.slot[@intCast(c_uint, slot)].type;
    if ((@ptrCast(?*c_void, (type_1))) != @intToPtr(?*c_void, @as(c_int, 0))) type_1.?.* = t;
    if ((@ptrCast(?*c_void, (val))) != @intToPtr(?*c_void, @as(c_int, 0))) val.?.* = d.*.slot[@intCast(c_uint, slot)].val;
    return (if (@bitCast(c_uint, @enumToInt(t)) == @bitCast(c_uint, ONIG_TYPE_VOID)) @as(c_int, 1) else @as(c_int, 0));
}
pub fn onig_get_callout_data_by_callout_args_self_dont_clear_old(arg_args: [*c]OnigCalloutArgs, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var slot = arg_slot;
    var type_1 = arg_type_1;
    var val = arg_val;
    return onig_get_callout_data_dont_clear_old(args.*.regex, args.*.msa.*.mp, args.*.num, slot, type_1, val);
}
pub fn onig_get_callout_data(arg_reg: OnigRegex, arg_mp: [*c]OnigMatchParam, arg_callout_num: c_int, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    var callout_num = arg_callout_num;
    var slot = arg_slot;
    ...
    if (d.*.last_match_at_call_counter != mp.*.match_at_call_counter) {
        _ = __builtin___memset_chk(@ptrCast(?*c_void, d), @as(c_int, 0), @sizeOf(CalloutData), __builtin_object_size(@ptrCast(?*const c_void, d), @as(c_int, 0)));
...
pub fn onig_get_callout_data_by_callout_args(arg_args: [*c]OnigCalloutArgs, arg_callout_num: c_int, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var callout_num = arg_callout_num;
    var slot = arg_slot;
    var type_1 = arg_type_1;
    var val = arg_val;
    return onig_get_callout_data(args.*.regex, args.*.msa.*.mp, callout_num, slot, type_1, val);
}
pub fn onig_get_callout_data_by_callout_args_self(arg_args: [*c]OnigCalloutArgs, arg_slot: c_int, arg_type_1: [*c]OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var slot = arg_slot;
    var type_1 = arg_type_1;
    var val = arg_val;
    return onig_get_callout_data(args.*.regex, args.*.msa.*.mp, args.*.num, slot, type_1, val);
}
pub fn onig_set_callout_data(arg_reg: OnigRegex, arg_mp: [*c]OnigMatchParam, arg_callout_num: c_int, arg_slot: c_int, arg_type_1: OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    var callout_num = arg_callout_num;
    var slot = arg_slot;
    ...
    if (callout_num <= @as(c_int, 0)) return -@as(c_int, 30);
    d = ((mp).*.callout_data + ((callout_num) - @as(c_int, 1)));
    d.*.slot[@intCast(c_uint, slot)].type = type_1;
    d.*.slot[@intCast(c_uint, slot)].val = val.?.*;
    d.*.last_match_at_call_counter = mp.*.match_at_call_counter;
    return 0;
}
pub fn onig_set_callout_data_by_callout_args(arg_args: [*c]OnigCalloutArgs, arg_callout_num: c_int, arg_slot: c_int, arg_type_1: OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var callout_num = arg_callout_num;
    var slot = arg_slot;
    var type_1 = arg_type_1;
    var val = arg_val;
    return onig_set_callout_data(args.*.regex, args.*.msa.*.mp, callout_num, slot, type_1, val);
}
pub fn onig_set_callout_data_by_callout_args_self(arg_args: [*c]OnigCalloutArgs, arg_slot: c_int, arg_type_1: OnigType, arg_val: [*c]OnigValue) callconv(.C) c_int {
    var args = arg_args;
    var slot = arg_slot;
    var type_1 = arg_type_1;
    var val = arg_val;
    return onig_set_callout_data(args.*.regex, args.*.msa.*.mp, args.*.num, slot, type_1, val);
}
pub fn onig_get_capture_range_in_callout(arg_a: [*c]OnigCalloutArgs, arg_mem_num: c_int, arg_begin: [*c]c_int, arg_end: [*c]c_int) callconv(.C) c_int {
    var a = arg_a;
    var mem_num = arg_mem_num;
    var begin = arg_begin;
    var end = arg_end;
    var reg: OnigRegex = undefined;
    var str: [*c]const OnigUChar = undefined;
    var stk_base: [*c]StackType = undefined;
    var i: c_int = undefined;
    var mem_start_stk: [*c]StkPtrType = undefined;
    var mem_end_stk: [*c]StkPtrType = undefined;
    i = mem_num;
    reg = a.*.regex;
    str = a.*.string;
    stk_base = a.*.stk_base;
    ...
    if (i > @as(c_int, 0)) {
        if (a.*.mem_end_stk[@intCast(c_uint, i)].i != (@bitCast(StackIndex, @as(c_long, -@as(c_int, 1))))) {
            begin.?.* = @bitCast(c_int, @truncate(c_int, ((if ((if ((i) < @bitCast(c_int, @truncate(c_uint, (@sizeOf(MemStatusType) *% @bitCast(c_ulong, @as(c_long, @as(c_int, 8))))))) (((reg).*.push_mem_start) & (@bitCast(MemStatusType, @as(c_int, 1)) << @intCast(@import("std").math.Log2Int(MemStatusType), (i)))) else (((reg).*.push_mem_start) & @bitCast(c_uint, @as(c_int, 1)))) != @bitCast(c_uint, @as(c_int, 0))) (stk_base + (mem_start_stk[@intCast(c_uint, i)].i)).*.u.mem.pstr else mem_start_stk[@intCast(c_uint, i)].s) - str)));
            end.?.* = @bitCast(c_int, @truncate(c_int, ((if ((if ((i) < @bitCast(c_int, @truncate(c_uint, (@sizeOf(MemStatusType) *% @bitCast(c_ulong, @as(c_long, @as(c_int, 8))))))) (((reg).*.push_mem_end) & (@bitCast(MemStatusType, @as(c_int, 1)) << @intCast(@import("std").math.Log2Int(MemStatusType), (i)))) else (((reg).*.push_mem_end) & @bitCast(c_uint, @as(c_int, 1)))) != @bitCast(c_uint, @as(c_int, 0))) (stk_base + (mem_end_stk[@intCast(c_uint, i)].i)).*.u.mem.pstr else mem_end_stk[@intCast(c_uint, i)].s) - str)));
        } else {
            begin.?.* = blk: {
                const tmp = -@as(c_int, 1);
                end.?.* = tmp;
    ...
    } else return -@as(c_int, 30);
    return 0;
}
pub fn onig_get_used_stack_size_in_callout(arg_a: [*c]OnigCalloutArgs, arg_used_num: [*c]c_int, arg_used_bytes: [*c]c_int) callconv(.C) c_int {
    var a = arg_a;
    var used_num = arg_used_num;
    var used_bytes = arg_used_bytes;
    var n: c_int = undefined;
    n = @bitCast(c_int, @truncate(c_int, (a.*.stk - a.*.stk_base)));
    if (used_num != null) used_num.?.* = n;
    if (used_bytes != null) used_bytes.?.* = @bitCast(c_int, @truncate(c_uint, (@bitCast(c_ulong, @as(c_long, n)) *% @sizeOf(StackType))));
    return 0;
}
pub fn onig_builtin_fail(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    return ONIG_CALLOUT_FAIL;
}
pub fn onig_builtin_mismatch(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    return -@as(c_int, 1);
}
pub fn onig_builtin_error(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    var r: c_int = undefined;
    var n: c_int = undefined;
    var val: OnigValue = undefined;
    r = onig_get_arg_by_callout_args(args, @as(c_int, 0), null, &val);
    if (r != @as(c_int, 0)) return r;
    n = @bitCast(c_int, @truncate(c_int, val.l));
    if (n >= @as(c_int, 0)) {
        n = -@as(c_int, 230);
    } else if (onig_is_error_code_needs_param(n) != 0) {
        n = -@as(c_int, 230);
    }
    return n;
}
pub fn onig_builtin_count(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    _ = onig_check_callout_data_and_clear_old_values(args);
    return onig_builtin_total_count(args, user_data);
}
pub fn onig_builtin_total_count(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    var r: c_int = undefined;
    var slot: c_int = undefined;
    var type_1: OnigType = undefined;
    var val: OnigValue = undefined;
    var aval: OnigValue = undefined;
    var count_type: OnigCodePoint = undefined;
    r = onig_get_arg_by_callout_args(args, @as(c_int, 0), &type_1, &aval);
    if (r != @as(c_int, 0)) return r;
    count_type = aval.c;
    if (((count_type != @bitCast(c_uint, @as(c_int, '>'))) and (count_type != @bitCast(c_uint, @as(c_int, 'X')))) and (count_type != @bitCast(c_uint, @as(c_int, '<')))) return -@as(c_int, 232);
    r = onig_get_callout_data_by_callout_args_self_dont_clear_old(args, @as(c_int, 0), &type_1, &val);
    if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) {
        val.l = @bitCast(c_long, @as(c_long, @as(c_int, 0)));
    }
    if (@bitCast(c_uint, @enumToInt(args.*.in)) == @bitCast(c_uint, ONIG_CALLOUT_IN_RETRACTION)) {
        slot = 2;
        if (count_type == @bitCast(c_uint, @as(c_int, '<'))) val.l += 1 else if (count_type == @bitCast(c_uint, @as(c_int, 'X'))) val.l -= 1;
    } else {
        slot = 1;
        if (count_type != @bitCast(c_uint, @as(c_int, '<'))) val.l += 1;
    }
    r = onig_set_callout_data_by_callout_args_self(args, @as(c_int, 0), @intToEnum(OnigType, ONIG_TYPE_LONG), &val);
    if (r != @as(c_int, 0)) return r;
    r = onig_get_callout_data_by_callout_args_self_dont_clear_old(args, slot, &type_1, &val);
    if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) {
        val.l = @bitCast(c_long, @as(c_long, @as(c_int, 0)));
    }
    val.l += 1;
    r = onig_set_callout_data_by_callout_args_self(args, slot, @intToEnum(OnigType, ONIG_TYPE_LONG), &val);
    if (r != @as(c_int, 0)) return r;
    return ONIG_CALLOUT_SUCCESS;
}
pub fn onig_builtin_max(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    var r: c_int = undefined;
    var slot: c_int = undefined;
    ...
    _ = onig_check_callout_data_and_clear_old_values(args);
    slot = 0;
    r = onig_get_callout_data_by_callout_args_self(args, slot, &type_1, &val);
    if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) {
        type_1 = @intToEnum(OnigType, ONIG_TYPE_LONG);
        val.l = @bitCast(c_long, @as(c_long, @as(c_int, 0)));
    }
    r = onig_get_arg_by_callout_args(args, @as(c_int, 0), &type_1, &aval);
    if (r != @as(c_int, 0)) return r;
    if (@bitCast(c_uint, @enumToInt(type_1)) == @bitCast(c_uint, ONIG_TYPE_TAG)) {
        r = onig_get_callout_data_by_callout_args(args, aval.tag, @as(c_int, 0), &type_1, &aval);
        if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) max_val = 0 else max_val = aval.l;
    } else {
        max_val = aval.l;
    }
    r = onig_get_arg_by_callout_args(args, @as(c_int, 1), &type_1, &aval);
    if (r != @as(c_int, 0)) return r;
    count_type = aval.c;
    if (((count_type != @bitCast(c_uint, @as(c_int, '>'))) and (count_type != @bitCast(c_uint, @as(c_int, 'X')))) and (count_type != @bitCast(c_uint, @as(c_int, '<')))) return -@as(c_int, 232);
    if (@bitCast(c_uint, @enumToInt(args.*.in)) == @bitCast(c_uint, ONIG_CALLOUT_IN_RETRACTION)) {
        if (count_type == @bitCast(c_uint, @as(c_int, '<'))) {
            if (val.l >= max_val) return ONIG_CALLOUT_FAIL;
            val.l += 1;
        } else if (count_type == @bitCast(c_uint, @as(c_int, 'X'))) val.l -= 1;
    } else {
        if (count_type != @bitCast(c_uint, @as(c_int, '<'))) {
            if (val.l >= max_val) return ONIG_CALLOUT_FAIL;
            val.l += 1;
...
pub const OP_EQ = @enumToInt(enum_OP_CMP.OP_EQ);
pub const OP_NE = @enumToInt(enum_OP_CMP.OP_NE);
pub const OP_LT = @enumToInt(enum_OP_CMP.OP_LT);
pub const OP_GT = @enumToInt(enum_OP_CMP.OP_GT);
pub const OP_LE = @enumToInt(enum_OP_CMP.OP_LE);
pub const OP_GE = @enumToInt(enum_OP_CMP.OP_GE);
pub const enum_OP_CMP = extern enum(c_int) {
    OP_EQ,
    OP_NE,
    OP_LT,
    OP_GT,
...
pub fn onig_builtin_cmp(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    var r: c_int = undefined;
    var slot: c_int = undefined;
    var lv: c_long = undefined;
    var rv: c_long = undefined;
    var type_1: OnigType = undefined;
    var val: OnigValue = undefined;
    var reg: [*c]regex_t = undefined;
    var op: enum_OP_CMP = undefined;
    reg = args.*.regex;
    r = onig_get_arg_by_callout_args(args, @as(c_int, 0), &type_1, &val);
    if (r != @as(c_int, 0)) return r;
    if (@bitCast(c_uint, @enumToInt(type_1)) == @bitCast(c_uint, ONIG_TYPE_TAG)) {
        r = onig_get_callout_data_by_callout_args(args, val.tag, @as(c_int, 0), &type_1, &val);
        if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) lv = 0 else lv = val.l;
    } else {
        lv = val.l;
    }
    r = onig_get_arg_by_callout_args(args, @as(c_int, 2), &type_1, &val);
    if (r != @as(c_int, 0)) return r;
    if (@bitCast(c_uint, @enumToInt(type_1)) == @bitCast(c_uint, ONIG_TYPE_TAG)) {
        r = onig_get_callout_data_by_callout_args(args, val.tag, @as(c_int, 0), &type_1, &val);
        if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) rv = 0 else rv = val.l;
    } else {
        rv = val.l;
    }
    slot = 0;
    r = onig_get_callout_data_by_callout_args_self(args, slot, &type_1, &val);
    if (r < @as(c_int, 0)) return r else if (r > @as(c_int, 0)) {
        var c1: OnigCodePoint = undefined;
        var c2: OnigCodePoint = undefined;
        var p: [*c]OnigUChar = undefined;
        r = onig_get_arg_by_callout_args(args, @as(c_int, 1), &type_1, &val);
        if (r != @as(c_int, 0)) return r;
        p = val.s.start;
        c1 = (reg.*.enc).*.mbc_to_code.?((p), (val.s.end));
        p += (reg.*.enc).*.mbc_enc_len.?(p);
        if (p < val.s.end) {
            c2 = (reg.*.enc).*.mbc_to_code.?((p), (val.s.end));
            p += (reg.*.enc).*.mbc_enc_len.?(p);
            if (p != val.s.end) return -@as(c_int, 232);
        } else c2 = @bitCast(OnigCodePoint, @as(c_int, 0));
        switch_7: {
            default: {
                case_5: {
                    case_3: {
                        case_1: {
                            case: {
                                switch (c1) {
                                    @bitCast(OnigCodePoint, @as(c_int, 61)) => break :case,
                                    @bitCast(OnigCodePoint, @as(c_int, 33)) => break :case_1,
                                    @bitCast(OnigCodePoint, @as(c_int, 60)) => break :case_3,
                                    @bitCast(OnigCodePoint, @as(c_int, 62)) => break :case_5,
                                    else => break :default,
                                }
                            }
                            if (c2 != @bitCast(c_uint, @as(c_int, '='))) return -@as(c_int, 232);
                            op = @intToEnum(enum_OP_CMP, OP_EQ);
                            break :@"switch";
                        }
                        if (c2 != @bitCast(c_uint, @as(c_int, '='))) return -@as(c_int, 232);
                        op = @intToEnum(enum_OP_CMP, OP_NE);
                        break :switch_2;
                    }
                    if (c2 == @bitCast(c_uint, @as(c_int, '='))) op = @intToEnum(enum_OP_CMP, OP_LE) else if (c2 == @bitCast(c_uint, @as(c_int, 0))) op = @intToEnum(enum_OP_CMP, OP_LT) else return -@as(c_int, 232);
                    break :switch_4;
                }
                if (c2 == @bitCast(c_uint, @as(c_int, '='))) op = @intToEnum(enum_OP_CMP, OP_GE) else if (c2 == @bitCast(c_uint, @as(c_int, 0))) op = @intToEnum(enum_OP_CMP, OP_GT) else return -@as(c_int, 232);
                break :switch_6;
            }
            return -@as(c_int, 232);
            break :switch_7;
        }
        val.l = @bitCast(c_long, @as(c_long, @enumToInt(op)));
        r = onig_set_callout_data_by_callout_args_self(args, slot, @intToEnum(OnigType, ONIG_TYPE_LONG), &val);
        if (r != @as(c_int, 0)) return r;
    } else {
        op = @intToEnum(enum_OP_CMP, val.l);
    }
    @"switch": {
        case_9: {
            case_7: {
                case_5: {
                    case_3: {
                        case_1: {
                            case: {
                                switch (@bitCast(c_uint, @enumToInt(op))) {
    ...
    return if (r == @as(c_int, 0)) ONIG_CALLOUT_FAIL else ONIG_CALLOUT_SUCCESS;
}
pub fn onig_setup_builtin_monitors_by_ascii_encoded_name(arg_fp: ?*c_void) callconv(.C) c_int {
    var fp = arg_fp;
    var id: c_int = undefined;
    var name: [*c]u8 = undefined;
    var enc: OnigEncoding = undefined;
    var ts: [4]c_uint = undefined;
    var opts: [4]OnigValue = undefined;
    if ((fp) != @intToPtr(?*c_void, @as(c_int, 0))) OutFp = @ptrCast([*c]FILE, @alignCast(@alignOf(FILE), fp)) else OutFp = __stdoutp;
    enc = (&OnigEncodingASCII);
    name = "MON";
    ts[@intCast(c_uint, @as(c_int, 0))] = @bitCast(c_uint, ONIG_TYPE_CHAR);
    opts[@intCast(c_uint, @as(c_int, 0))].c = @bitCast(OnigCodePoint, @as(c_int, '>'));
    while (true) {
        var len: c_int = onigenc_str_bytelen_null(enc, @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), name)));
        id = onig_set_callout_of_name(enc, @intToEnum(OnigCalloutType, ONIG_CALLOUT_TYPE_SINGLE), @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (name))), @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), ((name) + len))), (ONIG_CALLOUT_IN_PROGRESS | ONIG_CALLOUT_IN_RETRACTION), onig_builtin_monitor, null, (@as(c_int, 1)), &(ts), (@as(c_int, 1)), &(opts));
...
pub const forward_search = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:5110:1
// ../oniguruma/src/regexec.c:5377:5: warning: TODO implement translation of stmt class GotoStmtClass
pub const search_in_range = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:5355:1
const struct_unnamed_76 = extern struct {
    type: OnigType,
    val: OnigValue,
};
const struct_unnamed_75 = extern struct {
    last_match_at_call_counter: c_int,
    slot: [5]struct_unnamed_76,
};
pub const CalloutData = struct_unnamed_75;
const struct_unnamed_77 = extern struct {
    ...
    retry_limit_in_match: c_ulong,
    retry_limit_in_search: c_ulong,
    retry_limit_in_search_counter: c_ulong,
    mp: [*c]OnigMatchParam,
    best_len: c_int,
    best_s: [*c]OnigUChar,
    subexp_call_in_search_counter: c_ulong,
};
pub const MatchArg = struct_unnamed_77;
pub fn history_tree_free(arg_node: [*c]OnigCaptureTreeNode) callconv(.C) void {
    var node = arg_node;
    history_tree_clear(node);
    if ((@ptrCast(?*c_void, (node.*.childs))) != @intToPtr(?*c_void, @as(c_int, 0))) free(@ptrCast(?*c_void, node.*.childs));
    free(@ptrCast(?*c_void, node));
}
pub fn history_tree_clear(arg_node: [*c]OnigCaptureTreeNode) callconv(.C) void {
    var node = arg_node;
    var i: c_int = undefined;
    if ((@ptrCast(?*c_void, (node))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
    {
        i = 0;
        while (i < node.*.num_childs) : (i += 1) {
            if ((@ptrCast(?*c_void, (node.*.childs[@intCast(c_uint, i)]))) != @intToPtr(?*c_void, @as(c_int, 0))) {
                history_tree_free(node.*.childs[@intCast(c_uint, i)]);
            }
        }
    }
    {
        i = 0;
        while (i < node.*.allocated) : (i += 1) {
            node.*.childs[@intCast(c_uint, i)] = @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0));
        }
    }
    node.*.num_childs = 0;
    node.*.beg = -@as(c_int, 1);
    node.*.end = -@as(c_int, 1);
    node.*.group = -@as(c_int, 1);
}
pub fn history_root_free(arg_r: [*c]OnigRegion) callconv(.C) void {
    var r = arg_r;
    if ((@ptrCast(?*c_void, (r.*.history_root))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
    history_tree_free(r.*.history_root);
    r.*.history_root = @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0));
}
pub fn history_node_new() callconv(.C) [*c]OnigCaptureTreeNode {
    var node: [*c]OnigCaptureTreeNode = undefined;
    node = @ptrCast([*c]OnigCaptureTreeNode, @alignCast(@alignOf(OnigCaptureTreeNode), malloc(@sizeOf(OnigCaptureTreeNode))));
    if ((@ptrCast(?*c_void, (node))) == @intToPtr(?*c_void, @as(c_int, 0))) return null;
    node.*.childs = @intToPtr([*c][*c]OnigCaptureTreeNode, @as(c_int, 0));
    node.*.allocated = 0;
    node.*.num_childs = 0;
    node.*.group = -@as(c_int, 1);
    node.*.beg = -@as(c_int, 1);
...
pub fn history_tree_add_child(arg_parent: [*c]OnigCaptureTreeNode, arg_child: [*c]OnigCaptureTreeNode) callconv(.C) c_int {
    var parent = arg_parent;
    var child = arg_child;
    if (parent.*.num_childs >= parent.*.allocated) {
        var n: c_int = undefined;
        var i: c_int = undefined;
        if ((@ptrCast(?*c_void, (parent.*.childs))) == @intToPtr(?*c_void, @as(c_int, 0))) {
            n = 8;
            parent.*.childs = @ptrCast([*c][*c]OnigCaptureTreeNode, @alignCast(@alignOf([*c]OnigCaptureTreeNode), malloc((@sizeOf([*c]struct_OnigCaptureTreeNodeStruct) *% @bitCast(c_ulong, @as(c_long, n))))));
        } else {
            n = (parent.*.allocated * @as(c_int, 2));
            parent.*.childs = @ptrCast([*c][*c]OnigCaptureTreeNode, @alignCast(@alignOf([*c]OnigCaptureTreeNode), realloc(@ptrCast(?*c_void, parent.*.childs), (@sizeOf([*c]struct_OnigCaptureTreeNodeStruct) *% @bitCast(c_ulong, @as(c_long, n))))));
        }
        if ((@ptrCast(?*c_void, (parent.*.childs))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
        {
            i = parent.*.allocated;
            while (i < n) : (i += 1) {
                parent.*.childs[@intCast(c_uint, i)] = @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0));
            }
        }
        parent.*.allocated = n;
    }
    parent.*.childs[@intCast(c_uint, parent.*.num_childs)] = child;
    parent.*.num_childs += 1;
    return 0;
}
pub fn history_tree_clone(arg_node: [*c]OnigCaptureTreeNode) callconv(.C) [*c]OnigCaptureTreeNode {
    var node = arg_node;
    var i: c_int = undefined;
    var clone: [*c]OnigCaptureTreeNode = undefined;
    var child: [*c]OnigCaptureTreeNode = undefined;
    clone = history_node_new();
    if ((@ptrCast(?*c_void, (clone))) == @intToPtr(?*c_void, @as(c_int, 0))) return null;
    clone.*.beg = node.*.beg;
    clone.*.end = node.*.end;
    {
        i = 0;
        while (i < node.*.num_childs) : (i += 1) {
            child = history_tree_clone(node.*.childs[@intCast(c_uint, i)]);
            if ((@ptrCast(?*c_void, (child))) == @intToPtr(?*c_void, @as(c_int, 0))) {
                history_tree_free(clone);
                return @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0));
            }
            _ = history_tree_add_child(clone, child);
        }
    }
    return clone;
}
pub fn onig_region_resize_clear(arg_region: [*c]OnigRegion, arg_n: c_int) callconv(.C) c_int {
    var region = arg_region;
    var n = arg_n;
    var r: c_int = undefined;
...
pub const StackIndex = ptrdiff_t;
const union_unnamed_78 = extern union {
    i: StackIndex,
    s: [*c]OnigUChar,
};
pub const StkPtrType = union_unnamed_78;
...
const struct_unnamed_86 = extern struct {
    num: c_int,
    func: OnigCalloutFunc,
};
const union_unnamed_79 = extern union {
    state: struct_unnamed_80,
    repeat_inc: struct_unnamed_81,
    mem: struct_unnamed_82,
    empty_check: struct_unnamed_83,
    call_frame: struct_unnamed_84,
    val: struct_unnamed_85,
    callout: struct_unnamed_86,
};
pub const struct__StackType = extern struct {
    type: c_uint,
    zid: c_int,
    u: union_unnamed_79,
};
pub const StackType = struct__StackType;
pub var MatchStackLimit: c_uint = @bitCast(c_uint, @as(c_int, 0));
pub var RetryLimitInMatch: c_ulong = @bitCast(c_ulong, @as(c_long, @as(c_int, 10000000)));
pub var RetryLimitInSearch: c_ulong = @bitCast(c_ulong, @as(c_long, @as(c_int, 0)));
pub var SubexpCallLimitInSearch: c_ulong = @bitCast(c_ulong, @as(c_long, @as(c_int, 0)));
pub var DefaultProgressCallout: OnigCalloutFunc = @import("std").mem.zeroes(OnigCalloutFunc);
pub var DefaultRetractionCallout: OnigCalloutFunc = @import("std").mem.zeroes(OnigCalloutFunc);
pub fn adjust_match_param(arg_reg: [*c]regex_t, arg_mp: [*c]OnigMatchParam) callconv(.C) c_int {
    var reg = arg_reg;
    var mp = arg_mp;
    var ext: [*c]RegexExt = reg.*.extp;
    mp.*.match_at_call_counter = 0;
    if (((@ptrCast(?*c_void, (ext))) == @intToPtr(?*c_void, @as(c_int, 0))) or (ext.*.callout_num == @as(c_int, 0))) return 0;
    if (ext.*.callout_num > mp.*.callout_data_alloc_num) {
        var d: [*c]CalloutData = undefined;
        var n: usize = (@bitCast(c_ulong, @as(c_long, ext.*.callout_num)) *% @sizeOf(CalloutData));
        if ((@ptrCast(?*c_void, (mp.*.callout_data))) != @intToPtr(?*c_void, @as(c_int, 0))) d = @ptrCast([*c]CalloutData, @alignCast(@alignOf(CalloutData), realloc(@ptrCast(?*c_void, mp.*.callout_data), n))) else d = @ptrCast([*c]CalloutData, @alignCast(@alignOf(CalloutData), malloc(n)));
        if ((@ptrCast(?*c_void, (d))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
        mp.*.callout_data = d;
        mp.*.callout_data_alloc_num = ext.*.callout_num;
    }
    _ = __builtin___memset_chk(@ptrCast(?*c_void, mp.*.callout_data), @as(c_int, 0), (@bitCast(c_ulong, @as(c_long, mp.*.callout_data_alloc_num)) *% @sizeOf(CalloutData)), __builtin_object_size(@ptrCast(?*const c_void, mp.*.callout_data), @as(c_int, 0)));
    return 0;
}
pub fn onig_check_callout_data_and_clear_old_values(arg_args: [*c]OnigCalloutArgs) callconv(.C) c_int {
    var args = arg_args;
    var mp: [*c]OnigMatchParam = undefined;
    var num: c_int = undefined;
    var d: [*c]CalloutData = undefined;
    mp = args.*.msa.*.mp;
    num = args.*.num;
    d = ((mp).*.callout_data + ((num) - @as(c_int, 1)));
    if (d.*.last_match_at_call_counter != mp.*.match_at_call_counter) {
        _ = __builtin___memset_chk(@ptrCast(?*c_void, d), @as(c_int, 0), @sizeOf(CalloutData), __builtin_object_size(@ptrCast(?*const c_void, d), @as(c_int, 0)));
        d.*.last_match_at_call_counter = mp.*.match_at_call_counter;
...
pub fn stack_double(arg_is_alloca: [*c]c_int, arg_arg_alloc_base: [*c][*c]u8, arg_arg_stk_base: [*c][*c]StackType, arg_arg_stk_end: [*c][*c]StackType, arg_arg_stk: [*c][*c]StackType, arg_msa: [*c]MatchArg) callconv(.C) c_int {
    var is_alloca = arg_is_alloca;
    var arg_alloc_base = arg_arg_alloc_base;
    var arg_stk_base = arg_arg_stk_base;
    var arg_stk_end = arg_arg_stk_end;
    var arg_stk = arg_arg_stk;
    var msa = arg_msa;
    var n: c_uint = undefined;
    var used: c_int = undefined;
    var size: usize = undefined;
    var new_size: usize = undefined;
    var alloc_base: [*c]u8 = undefined;
    var new_alloc_base: [*c]u8 = undefined;
    var stk_base: [*c]StackType = undefined;
    var stk_end: [*c]StackType = undefined;
    var stk: [*c]StackType = undefined;
    alloc_base = arg_alloc_base.?.*;
    stk_base = arg_stk_base.?.*;
    stk_end = arg_stk_end.?.*;
    stk = arg_stk.?.*;
    n = @bitCast(c_uint, @truncate(c_int, (stk_end - stk_base)));
    size = ((@sizeOf(StkPtrType) *% @bitCast(c_ulong, @as(c_long, msa.*.ptr_num))) +% (@sizeOf(StackType) *% @bitCast(c_ulong, @as(c_ulong, n))));
    n *%= @bitCast(c_uint, @as(c_int, 2));
    new_size = ((@sizeOf(StkPtrType) *% @bitCast(c_ulong, @as(c_long, msa.*.ptr_num))) +% (@sizeOf(StackType) *% @bitCast(c_ulong, @as(c_ulong, n))));
    if (is_alloca.?.* != @as(c_int, 0)) {
        new_alloc_base = @ptrCast([*c]u8, @alignCast(@alignOf(u8), malloc(new_size)));
        if ((@ptrCast(?*c_void, (new_alloc_base))) == @intToPtr(?*c_void, @as(c_int, 0))) {
            while (true) {
                (msa).*.stack_n = @bitCast(c_int, @truncate(c_int, (stk_end - stk_base)));
                if ((is_alloca.?.*) != @as(c_int, 0)) {
                    var size_1: usize = ((@sizeOf(StkPtrType) *% @bitCast(c_ulong, @as(c_long, (msa).*.ptr_num))) +% (@sizeOf(StackType) *% @bitCast(c_ulong, @as(c_long, (msa).*.stack_n))));
                    (msa).*.stack_p = malloc(size_1);
                    if (((msa).*.stack_p) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
                    _ = __builtin___memcpy_chk((msa).*.stack_p, @ptrCast(?*const c_void, (alloc_base)), size_1, __builtin_object_size((msa).*.stack_p, @as(c_int, 0)));
                } else {
                    (msa).*.stack_p = @ptrCast(?*c_void, (alloc_base));
                }
                {}
                if (!false) break;
            }
        ...
        _ = __builtin___memcpy_chk(@ptrCast(?*c_void, new_alloc_base), @ptrCast(?*const c_void, alloc_base), size, __builtin_object_size(@ptrCast(?*const c_void, new_alloc_base), @as(c_int, 0)));
        is_alloca.?.* = 0;
    } else {
        if ((msa.*.match_stack_limit != @bitCast(c_uint, @as(c_int, 0))) and (n > msa.*.match_stack_limit)) {
            if (@bitCast(c_uint, @truncate(c_int, (stk_end - stk_base))) == msa.*.match_stack_limit) {
                while (true) {
                    (msa).*.stack_n = @bitCast(c_int, @truncate(c_int, (stk_end - stk_base)));
                    if ((is_alloca.?.*) != @as(c_int, 0)) {
                        var size_1: usize = ((@sizeOf(StkPtrType) *% @bitCast(c_ulong, @as(c_long, (msa).*.ptr_num))) +% (@sizeOf(StackType) *% @bitCast(c_ulong, @as(c_long, (msa).*.stack_n))));
                ...
                return -@as(c_int, 15);
            } else n = msa.*.match_stack_limit;
        }
        new_alloc_base = @ptrCast([*c]u8, @alignCast(@alignOf(u8), realloc(@ptrCast(?*c_void, alloc_base), new_size)));
        if ((@ptrCast(?*c_void, (new_alloc_base))) == @intToPtr(?*c_void, @as(c_int, 0))) {
            while (true) {
                (msa).*.stack_n = @bitCast(c_int, @truncate(c_int, (stk_end - stk_base)));
                if ((is_alloca.?.*) != @as(c_int, 0)) {
    ...
    used = @bitCast(c_int, @truncate(c_int, (stk - stk_base)));
    arg_alloc_base.?.* = alloc_base;
    arg_stk_base.?.* = @ptrCast([*c]StackType, @alignCast(@alignOf(StackType), (alloc_base + (@sizeOf(StkPtrType) *% @bitCast(c_ulong, @as(c_long, msa.*.ptr_num))))));
    arg_stk.?.* = (arg_stk_base.?.* + used);
    arg_stk_end.?.* = (arg_stk_base.?.* + n);
    return 0;
}
pub fn string_cmp_ic(arg_enc: OnigEncoding, arg_case_fold_flag: c_int, arg_s1: [*c]OnigUChar, arg_ps2: [*c][*c]OnigUChar, arg_mblen_1: c_int) callconv(.C) c_int {
    var enc = arg_enc;
    var case_fold_flag = arg_case_fold_flag;
    var s1 = arg_s1;
    var ps2 = arg_ps2;
    var mblen_1 = arg_mblen_1;
    var buf1: [18]OnigUChar = undefined;
    var buf2: [18]OnigUChar = undefined;
    var p1: [*c]OnigUChar = undefined;
    var p2: [*c]OnigUChar = undefined;
    var end1: [*c]OnigUChar = undefined;
    var s2: [*c]OnigUChar = undefined;
    var end2: [*c]OnigUChar = undefined;
    var len1: c_int = undefined;
    var len2: c_int = undefined;
    s2 = ps2.?.*;
    end1 = (s1 + mblen_1);
    end2 = (s2 + mblen_1);
    while (s1 < end1) {
        len1 = (enc).*.mbc_case_fold.?(@bitCast(OnigCaseFoldType, case_fold_flag), @ptrCast([*c][*c]const OnigUChar, @alignCast(@alignOf([*c]const OnigUChar), &s1)), end1, &buf1);
        len2 = (enc).*.mbc_case_fold.?(@bitCast(OnigCaseFoldType, case_fold_flag), @ptrCast([*c][*c]const OnigUChar, @alignCast(@alignOf([*c]const OnigUChar), &s2)), end2, &buf2);
        ...
        while ((blk: {
            ...
            if (@bitCast(c_int, @as(c_uint, p1.?.*)) != @bitCast(c_int, @as(c_uint, p2.?.*))) return 0;
            p1 += 1;
            p2 += 1;
        }
        if (s2 >= end2) {
            if (s1 < end1) return 0 else break;
        }
    }
    ps2.?.* = s2;
    return 1;
}
pub fn make_capture_history_tree(arg_node: [*c]OnigCaptureTreeNode, arg_kp: [*c][*c]StackType, arg_stk_top: [*c]StackType, arg_str: [*c]OnigUChar, arg_reg: [*c]regex_t) callconv(.C) c_int {
    var node = arg_node;
    var kp = arg_kp;
    var stk_top = arg_stk_top;
    var str = arg_str;
    var reg = arg_reg;
    var n: c_int = undefined;
    var r: c_int = undefined;
    var child: [*c]OnigCaptureTreeNode = undefined;
    var k: [*c]StackType = kp.?.*;
    while (k < stk_top) {
        if (k.*.type == @bitCast(c_uint, @as(c_int, 16))) {
            n = k.*.zid;
            if ((n <= @as(c_int, 31)) and ((if ((n) < @bitCast(c_int, @truncate(c_uint, (@sizeOf(MemStatusType) *% @bitCast(c_ulong, @as(c_long, @as(c_int, 8))))))) ((reg.*.capture_history) & (@bitCast(MemStatusType, @as(c_int, 1)) << @intCast(@import("std").math.Log2Int(MemStatusType), n))) else ((reg.*.capture_history) & @bitCast(c_uint, @as(c_int, 1)))) != @bitCast(c_uint, @as(c_int, 0)))) {
                child = history_node_new();
                if ((@ptrCast(?*c_void, (child))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
                child.*.group = n;
                child.*.beg = @bitCast(c_int, @truncate(c_int, (k.*.u.mem.pstr - str)));
                ...
                if (r != @as(c_int, 0)) return r;
                k = kp.?.*;
                child.*.end = @bitCast(c_int, @truncate(c_int, (k.*.u.mem.pstr - str)));
            }
        } else if (k.*.type == @bitCast(c_uint, @as(c_int, 32816))) {
            if (k.*.zid == node.*.group) {
                node.*.end = @bitCast(c_int, @truncate(c_int, (k.*.u.mem.pstr - str)));
                kp.?.* = k;
                return 0;
            }
        }
...
pub fn mem_is_in_memp(arg_mem: c_int, arg_num: c_int, arg_memp: [*c]MemNumType) callconv(.C) c_int {
    var mem = arg_mem;
    var num = arg_num;
    var memp = arg_memp;
    var i: c_int = undefined;
    {
        i = 0;
        while (i < num) : (i += 1) {
            if (mem == @bitCast(c_int, memp[@intCast(c_uint, i)])) return 1;
        }
    }
    return 0;
}
pub fn backref_match_at_nested_level(arg_reg: [*c]regex_t, arg_top: [*c]StackType, arg_stk_base: [*c]StackType, arg_ignore_case: c_int, arg_case_fold_flag: c_int, arg_nest: c_int, arg_mem_num: c_int, arg_memp: [*c]MemNumType, arg_s: [*c][*c]OnigUChar, arg_send: [*c]const OnigUChar) callconv(.C) c_int {
    var reg = arg_reg;
    var top = arg_top;
    var stk_base = arg_stk_base;
    var ignore_case = arg_ignore_case;
    var case_fold_flag = arg_case_fold_flag;
    var nest = arg_nest;
    var mem_num = arg_mem_num;
    var memp = arg_memp;
    var s = arg_s;
    var send = arg_send;
    var ss: [*c]OnigUChar = undefined;
    var p: [*c]OnigUChar = undefined;
    var pstart: [*c]OnigUChar = undefined;
    var pend: [*c]OnigUChar = (@intToPtr([*c]OnigUChar, @as(c_int, 0)));
    var level: c_int = undefined;
    var k: [*c]StackType = undefined;
    level = 0;
    k = top;
    k -= 1;
    while (k >= stk_base) {
        if (k.*.type == @bitCast(c_uint, (@as(c_int, 1024) | @as(c_int, 16)))) {
            level -= 1;
        } else if (k.*.type == @bitCast(c_uint, (@as(c_int, 1280) | @as(c_int, 16)))) {
            level += 1;
        } else if (level == nest) {
            if (k.*.type == @bitCast(c_uint, @as(c_int, 16))) {
                if (mem_is_in_memp(k.*.zid, mem_num, memp) != 0) {
                    pstart = k.*.u.mem.pstr;
                    if ((@ptrCast(?*c_void, (pend))) != @intToPtr(?*c_void, @as(c_int, 0))) {
                        if ((pend - pstart) > (send - s.?.*)) return 0;
                        p = pstart;
                        ss = s.?.*;
                        if (ignore_case != @as(c_int, 0)) {
                            if (string_cmp_ic(reg.*.enc, case_fold_flag, pstart, &ss, @bitCast(c_int, @truncate(c_int, (pend - pstart)))) == @as(c_int, 0)) return 0;
                        } else {
                            while (p < pend) {
...
pub fn backref_check_at_nested_level(arg_reg: [*c]regex_t, arg_top: [*c]StackType, arg_stk_base: [*c]StackType, arg_nest: c_int, arg_mem_num: c_int, arg_memp: [*c]MemNumType) callconv(.C) c_int {
    var reg = arg_reg;
    var top = arg_top;
    var stk_base = arg_stk_base;
    var nest = arg_nest;
    ...
    while (k >= stk_base) {
        if (k.*.type == @bitCast(c_uint, (@as(c_int, 1024) | @as(c_int, 16)))) {
            ...
            if (k.*.type == @bitCast(c_uint, @as(c_int, 32816))) {
                if (mem_is_in_memp(k.*.zid, mem_num, memp) != 0) {
                    return 1;
                }
            }
...
pub var SubexpCallMaxNestLevel: c_int = 20;
pub const regoff_t = c_int;
const struct_unnamed_87 = extern struct {
    rm_so: regoff_t,
    rm_eo: regoff_t,
};
pub const posix_regmatch_t = struct_unnamed_87;
= extern struct { state:
    c_int,
,low
, high : [ *c]OnigUChar,
pub const match_at = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:2787:1
const struct_unnamed_88 = extern struct {
    reg: [*c]regex_t,
    region: [*c]OnigRegion,
};
pub const RR = struct_unnamed_88;
pub const SRS_DEAD = @enumToInt(enum_SearchRangeStatus.SRS_DEAD);
pub const SRS_LOW_HIGH = @enumToInt(enum_SearchRangeStatus.SRS_LOW_HIGH);
pub const SRS_ALL_RANGE = @enumToInt(enum_SearchRangeStatus.SRS_ALL_RANGE);
pub const enum_SearchRangeStatus = extern enum(c_int) {
    SRS_DEAD = 0,
    SRS_LOW_HIGH = 1,
    SRS_ALL_RANGE = 2,
    _,
...
const struct_unnamed_89 = extern struct {
    ...
    low: [*c]OnigUChar,
    high: [*c]OnigUChar,
    sch_range: [*c]OnigUChar,
};
pub const SearchRange = struct_unnamed_89;
pub const regset_search_body_position_lead = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:4432:1
pub fn regset_search_body_regex_lead(arg_set: [*c]OnigRegSet, arg_str: [*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_start: [*c]const OnigUChar, arg_orig_range: [*c]const OnigUChar, arg_lead: OnigRegSetLead, arg_option: OnigOptionType, arg_mps: [*c][*c]OnigMatchParam, arg_rmatch_pos: [*c]c_int) callconv(.C) c_int {
    var set = arg_set;
    var str = arg_str;
    var end = arg_end;
    var start = arg_start;
    var orig_range = arg_orig_range;
    var lead = arg_lead;
    var option = arg_option;
    var mps = arg_mps;
    var rmatch_pos = arg_rmatch_pos;
    ...
    var i: c_int = undefined;
    var n: c_int = undefined;
    var match_index: c_int = undefined;
    var ep: [*c]const OnigUChar = undefined;
    var reg: [*c]regex_t = undefined;
    var region: [*c]OnigRegion = undefined;
    n = set.*.n;
    match_index = -@as(c_int, 1);
    ep = orig_range;
    {
        i = 0;
        while (i < n) : (i += 1) {
            reg = set.*.rs[@intCast(c_uint, i)].reg;
            region = set.*.rs[@intCast(c_uint, i)].region;
            r = search_in_range(reg, str, end, start, ep, orig_range, region, option, mps[@intCast(c_uint, i)]);
            if (r > @as(c_int, 0)) {
                if ((str + r) < ep) {
                    match_index = i;
                    rmatch_pos.?.* = r;
                    if (@bitCast(c_uint, @enumToInt(lead)) == @bitCast(c_uint, ONIG_REGSET_PRIORITY_TO_REGEX_ORDER)) break;
                    ep = (str + r);
                }
            } else if (r == @as(c_int, 0)) {
                match_index = i;
                rmatch_pos.?.* = r;
                break;
            }
...
pub fn slow_search(arg_enc: OnigEncoding, arg_target: [*c]OnigUChar, arg_target_end: [*c]OnigUChar, arg_text: [*c]const OnigUChar, arg_text_end: [*c]const OnigUChar, arg_text_range: [*c]OnigUChar) callconv(.C) [*c]OnigUChar {
    var enc = arg_enc;
    var target = arg_target;
    var target_end = arg_target_end;
    var text = arg_text;
    var text_end = arg_text_end;
    var text_range = arg_text_range;
    var t: [*c]OnigUChar = undefined;
    var p: [*c]OnigUChar = undefined;
    var s: [*c]OnigUChar = undefined;
    var end: [*c]OnigUChar = undefined;
    end = @intToPtr([*c]OnigUChar, @ptrToInt(text_end));
    end -= ((target_end - target) - @bitCast(c_long, @as(c_long, @as(c_int, 1))));
    if (end > text_range) end = text_range;
    s = @intToPtr([*c]OnigUChar, @ptrToInt(text));
    while (s < end) {
        if (@bitCast(c_int, @as(c_uint, s.?.*)) == @bitCast(c_int, @as(c_uint, target.?.*))) {
            p = (s + @as(c_int, 1));
            t = (target + @as(c_int, 1));
            while (t < target_end) {
                if (@bitCast(c_int, @as(c_uint, t.?.*)) != @bitCast(c_int, @as(c_uint, (blk: {
                    const ref = &p;
                    const tmp = ref.*;
                    ref.* += 1;
                    break :blk tmp;
    ...
    return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
}
pub fn slow_search_backward(arg_enc: OnigEncoding, arg_target: [*c]OnigUChar, arg_target_end: [*c]OnigUChar, arg_text: [*c]const OnigUChar, arg_adjust_text: [*c]const OnigUChar, arg_text_end: [*c]const OnigUChar, arg_text_start: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var enc = arg_enc;
    var target = arg_target;
    var target_end = arg_target_end;
    var text = arg_text;
    var adjust_text = arg_adjust_text;
    var text_end = arg_text_end;
    var text_start = arg_text_start;
    var t: [*c]OnigUChar = undefined;
    var p: [*c]OnigUChar = undefined;
    var s: [*c]OnigUChar = undefined;
    s = @intToPtr([*c]OnigUChar, @ptrToInt(text_end));
    s -= (target_end - target);
    if (s > @intToPtr([*c]OnigUChar, @ptrToInt(text_start))) s = @intToPtr([*c]OnigUChar, @ptrToInt(text_start)) else s = (enc).*.left_adjust_char_head.?(adjust_text, s);
    while (((s) != @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))))) and ((s) >= @intToPtr([*c]OnigUChar, @ptrToInt((text))))) {
        if (@bitCast(c_int, @as(c_uint, s.?.*)) == @bitCast(c_int, @as(c_uint, target.?.*))) {
            p = (s + @as(c_int, 1));
            t = (target + @as(c_int, 1));
            while (t < target_end) {
...
pub fn sunday_quick_search_step_forward(arg_reg: [*c]regex_t, arg_target: [*c]const OnigUChar, arg_target_end: [*c]const OnigUChar, arg_text: [*c]const OnigUChar, arg_text_end: [*c]const OnigUChar, arg_text_range: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var reg = arg_reg;
    var target = arg_target;
    var target_end = arg_target_end;
    var text = arg_text;
    var text_end = arg_text_end;
    var text_range = arg_text_range;
    var s: [*c]const OnigUChar = undefined;
    var se: [*c]const OnigUChar = undefined;
    var t: [*c]const OnigUChar = undefined;
    var p: [*c]const OnigUChar = undefined;
    var end: [*c]const OnigUChar = undefined;
    var tail: [*c]const OnigUChar = undefined;
    var next: [*c]const OnigUChar = undefined;
    var skip: c_int = undefined;
    var tlen1: c_int = undefined;
    var map_offset: c_int = undefined;
    var enc: OnigEncoding = undefined;
    enc = reg.*.enc;
    tail = (target_end - @as(c_int, 1));
    tlen1 = @bitCast(c_int, @truncate(c_int, (tail - target)));
    end = text_range;
    if (@bitCast(c_long, @as(c_long, tlen1)) > (text_end - end)) end = (text_end - tlen1);
    ...
    while (s < end) {
        ...
        t = tail;
        while (@bitCast(c_int, @as(c_uint, p.?.*)) == @bitCast(c_int, @as(c_uint, t.?.*))) {
            if (t == target) return @intToPtr([*c]OnigUChar, @ptrToInt(s));
            p -= 1;
            t -= 1;
        }
        if ((se + map_offset) >= text_end) break;
        skip = @bitCast(c_int, @as(c_uint, reg.*.map[(se + map_offset).?.*]));
        next = (s + skip);
        if (next < end) s = onigenc_get_right_adjust_char_head(enc, s, next) else break;
    }
    return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
}
pub fn sunday_quick_search(arg_reg: [*c]regex_t, arg_target: [*c]const OnigUChar, arg_target_end: [*c]const OnigUChar, arg_text: [*c]const OnigUChar, arg_text_end: [*c]const OnigUChar, arg_text_range: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var reg = arg_reg;
    var target = arg_target;
    var target_end = arg_target_end;
    var text = arg_text;
    ...
    var tail: [*c]const OnigUChar = undefined;
    var map_offset: c_int = undefined;
    var target_len: ptrdiff_t = undefined;
    map_offset = reg.*.map_offset;
    tail = (target_end - @as(c_int, 1));
    target_len = (target_end - target);
    if (target_len > (text_end - text_range)) {
        end = text_end;
        if (target_len > (text_end - text)) return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
    } else {
        end = (text_range + target_len);
    }
    s = ((text + target_len) - @as(c_int, 1));
    if (s < end) {
        while (true) {
            p = s;
            t = tail;
            while (@bitCast(c_int, @as(c_uint, p.?.*)) == @bitCast(c_int, @as(c_uint, t.?.*))) {
                if (t == target) return @intToPtr([*c]OnigUChar, @ptrToInt(p));
                p -= 1;
                t -= 1;
            }
            if ((text_end - s) <= @bitCast(c_long, @as(c_long, map_offset))) break;
            if (@bitCast(c_long, @as(c_ulong, reg.*.map[(s + map_offset).?.*])) >= (end - s)) break;
            s += @bitCast(c_int, @as(c_uint, reg.*.map[(s + map_offset).?.*]));
        }
    }
    return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
}
pub fn map_search(arg_enc: OnigEncoding, arg_map: [*c]OnigUChar, arg_text: [*c]const OnigUChar, arg_text_range: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var enc = arg_enc;
    var map = arg_map;
    var text = arg_text;
    var text_range = arg_text_range;
    var s: [*c]const OnigUChar = text;
    while (s < text_range) {
        if (map[s.?.*] != 0) return @intToPtr([*c]OnigUChar, @ptrToInt(s));
        s += (enc).*.mbc_enc_len.?(s);
    }
    return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
}
pub fn map_search_backward(arg_enc: OnigEncoding, arg_map: [*c]OnigUChar, arg_text: [*c]const OnigUChar, arg_adjust_text: [*c]const OnigUChar, arg_text_start: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var enc = arg_enc;
    var map = arg_map;
    var text = arg_text;
    var adjust_text = arg_adjust_text;
    var text_start = arg_text_start;
    var s: [*c]const OnigUChar = text_start;
    while (((s) != @ptrCast([*c]const OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))))) and ((s) >= (text))) {
        if (map[s.?.*] != 0) return @intToPtr([*c]OnigUChar, @ptrToInt(s));
        s = onigenc_get_prev_char_head(enc, adjust_text, s);
    }
    return @ptrCast([*c]OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0)))));
} // ../oniguruma/src/regexec.c:5238:2: warning: TODO implement translation of stmt class LabelStmtClass
pub const backward_search = @compileError("unable to translate function"); // ../oniguruma/src/regexec.c:5231:1
pub fn update_regset_by_reg(arg_set: [*c]OnigRegSet, arg_reg: [*c]regex_t) callconv(.C) void {
    var set = arg_set;
    var reg = arg_reg;
    if (set.*.n == @as(c_int, 1)) {
        set.*.enc = reg.*.enc;
        ...
        set.*.all_low_high = if ((reg.*.optimize == OPTIMIZE_NONE) or (reg.*.dist_max == ~(@bitCast(OnigLen, @as(c_int, 0))))) @as(c_int, 0) else @as(c_int, 1);
        set.*.anychar_inf = if ((reg.*.anchor & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), 14))) != @as(c_int, 0)) @as(c_int, 1) else @as(c_int, 0);
    } else {
        var anchor: c_int = undefined;
        anchor = (set.*.anchor & reg.*.anchor);
        if (anchor != @as(c_int, 0)) {
            var anc_dmin: OnigLen = undefined;
            var anc_dmax: OnigLen = undefined;
        ...
        set.*.anchor = anchor;
        if ((reg.*.optimize == OPTIMIZE_NONE) or (reg.*.dist_max == ~(@bitCast(OnigLen, @as(c_int, 0))))) set.*.all_low_high = 0;
        if ((reg.*.anchor & (@as(c_int, 1) << @intCast(@import("std").math.Log2Int(c_int), 14))) != @as(c_int, 0)) set.*.anychar_inf = 1;
    }
}
pub fn onig_init_for_match_at(arg_reg: [*c]regex_t) callconv(.C) c_int {
    var reg = arg_reg;
    return match_at(reg, @ptrCast([*c]const OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0))))), @ptrCast([*c]const OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0))))), @ptrCast([*c]const OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0))))), @ptrCast([*c]const OnigUChar, @alignCast(@alignOf(OnigUChar), (@intToPtr(?*c_void, @as(c_int, 0))))), @ptrCast([*c]MatchArg, @alignCast(@alignOf(MatchArg), (@intToPtr(?*c_void, @as(c_int, 0))))));
}
pub var OutFp: [*c]FILE = @import("std").mem.zeroes([*c]FILE);
pub fn onig_builtin_monitor(arg_args: [*c]OnigCalloutArgs, arg_user_data: ?*c_void) callconv(.C) c_int {
    var args = arg_args;
    var user_data = arg_user_data;
    var r: c_int = undefined;
    var num: c_int = undefined;
    var tag_len: usize = undefined;
    var start: [*c]const OnigUChar = undefined;
    var right: [*c]const OnigUChar = undefined;
    var current: [*c]const OnigUChar = undefined;
    var string: [*c]const OnigUChar = undefined;
    var strend: [*c]const OnigUChar = undefined;
    var tag_start: [*c]const OnigUChar = undefined;
    var tag_end: [*c]const OnigUChar = undefined;
    var reg: [*c]regex_t = undefined;
    var in: OnigCalloutIn = undefined;
    var type_1: OnigType = undefined;
    var val: OnigValue = undefined;
    ...
    if (@bitCast(c_uint, @enumToInt(in)) == @bitCast(c_uint, ONIG_CALLOUT_IN_PROGRESS)) {
        if (val.c == @bitCast(c_uint, @as(c_int, '<'))) return ONIG_CALLOUT_SUCCESS;
    } else {
        if ((val.c != @bitCast(c_uint, @as(c_int, 'X'))) and (val.c != @bitCast(c_uint, @as(c_int, '<')))) return ONIG_CALLOUT_SUCCESS;
    }
    ...
    if (tag_start == null) _ = __builtin___snprintf_chk(&buf, @sizeOf([20]u8), @as(c_int, 0), __builtin_object_size(@ptrCast(?*const c_void, &buf), if (@as(c_int, 2) > @as(c_int, 1)) @as(c_int, 1) else @as(c_int, 0)), "#%d", num) else {
        var i: c_int = undefined;
        tag_len = @bitCast(usize, (tag_end - tag_start));
        if (tag_len >= @sizeOf([20]u8)) tag_len = (@sizeOf([20]u8) -% @bitCast(c_ulong, @as(c_long, @as(c_int, 1))));
        {
            i = 0;
            while (@bitCast(c_ulong, @as(c_long, i)) < tag_len) : (i += 1) buf[@intCast(c_uint, i)] = @bitCast(u8, tag_start[@intCast(c_uint, i)]);
        }
        buf[tag_len] = @bitCast(u8, @truncate(i8, @as(c_int, '\x00')));
    }
    _ = fprintf(fp, "ONIG-MONITOR: %-4s %s at: %d [%d - %d] len: %d\n", &buf, if (@bitCast(c_uint, @enumToInt(in)) == @bitCast(c_uint, ONIG_CALLOUT_IN_PROGRESS)) "=>" else "<=", @bitCast(c_int, @truncate(c_int, (current - string))), @bitCast(c_int, @truncate(c_int, (start - string))), @bitCast(c_int, @truncate(c_int, (right - string))), @bitCast(c_int, @truncate(c_int, (strend - string))));
    _ = fflush(fp);
    return ONIG_CALLOUT_SUCCESS;
}
...
pub const CALLOUT_BODY = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1023:9
pub const RETRACTION_CALLOUT = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1042:9
pub const UPDATE_FOR_STACK_REALLOC = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1204:9
pub const SAVE_REPEAT_STK_VAR = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:1209:9
pub const LOAD_TO_REPEAT_STK_VAR = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:1210:9
pub const SAVE_EMPTY_CHECK_STK_VAR = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:1213:9
pub const LOAD_TO_EMPTY_CHECK_STK_VAR = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:1214:9
pub const RETRY_IN_MATCH_ARG_INIT = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:1220:9
pub const SUBEXP_CALL_IN_MATCH_ARG_INIT = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:1229:9
pub const POP_CALL = @compileError("unable to translate C expr: unexpected token .Keyword_else"); // ../oniguruma/src/regexec.c:1232:9
pub const MATCH_ARG_INIT = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1239:9
pub const MATCH_ARG_FREE = @compileError("unable to translate C expr: unexpected token .Keyword_if"); // ../oniguruma/src/regexec.c:1265:9
pub const STACK_INIT = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1270:9
pub const STACK_SAVE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1302:9
pub const CHECK_RETRY_LIMIT_IN_MATCH = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1336:9
pub const ADJUST_MATCH_PARAM = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:1495:9
pub const STACK_ENSURE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1710:9
pub const STACK_PUSH_TYPE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1721:9
pub const STACK_PUSH = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1729:9
pub const STACK_PUSH_WITH_ZID = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1737:9
pub const STACK_PUSH_ENSURED = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1746:9
pub const STACK_PUSH_BOTTOM = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1760:9
pub const STACK_PUSH_REPEAT_INC = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1781:9
pub const STACK_PUSH_MEM_START = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1791:9
pub const STACK_PUSH_MEM_END = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1803:9
pub const STACK_PUSH_MEM_END_MARK = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1814:9
pub const STACK_GET_MEM_START = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1821:9
pub const STACK_GET_MEM_RANGE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1837:9
pub const STACK_PUSH_EMPTY_CHECK_START = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1855:9
pub const STACK_PUSH_EMPTY_CHECK_END = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1865:9
pub const STACK_PUSH_CALL_FRAME = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1872:9
pub const STACK_PUSH_RETURN = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1879:9
pub const STACK_PUSH_MARK = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1885:9
pub const STACK_PUSH_MARK_WITH_POS = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1892:9
pub const STACK_PUSH_SAVE_VAL = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1900:9
pub const STACK_PUSH_SAVE_VAL_WITH_SPREV = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1909:9
pub const STACK_GET_SAVE_VAL_TYPE_LAST = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1918:9
pub const STACK_GET_SAVE_VAL_TYPE_LAST_ID = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1930:9
pub const STACK_GET_SAVE_VAL_TYPE_LAST_ID_WITH_SPREV = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1951:9
pub const STACK_PUSH_CALLOUT_CONTENTS = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1971:9
pub const STACK_PUSH_CALLOUT_NAME = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1980:9
pub const STACK_BASE_CHECK = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:1996:9
pub const STACK_POP_ONE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:1999:9
pub const POP_CALLOUT_CASE = @compileError("unable to translate C expr: unexpected token .Keyword_else"); // ../oniguruma/src/regexec.c:2006:9
pub const STACK_POP = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2014:9
pub const STACK_POP_TO_MARK = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2058:9
pub const POP_TIL_BODY = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2085:9
pub const STACK_TO_VOID_TO_MARK = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2110:9
pub const EMPTY_CHECK_START_SEARCH = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2128:9
pub const STACK_EMPTY_CHECK = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2156:9
pub const STACK_MEM_START_GET_PREV_END_ADDR = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2162:9
pub const STACK_EMPTY_CHECK_MEM = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2175:9
pub const STACK_EMPTY_CHECK_MEM_REC = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2210:9
pub const STACK_GET_REPEAT_COUNT_SEARCH = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2303:9
pub const STACK_RETURN = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2343:9
pub const GET_STACK_RETURN_CALL = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2361:9
pub const STRING_CMP = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2381:9
pub const STRING_CMP_IC = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2387:9
pub const STRING_CMP_VALUE = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2424:9
pub const STRING_CMP_VALUE_IC = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2433:9
pub const DATA_ENSURE = @compileError("unable to translate C expr: unexpected token .Keyword_if"); // ../oniguruma/src/regexec.c:2446:9
pub const INIT_RIGHT_RANGE = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:2448:9
pub const STACK_INC = @compileError("TODO postfix inc/dec expr"); // ../oniguruma/src/regexec.c:2667:9
pub const SOP_IN = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:2669:9
pub const BYTECODE_INTERPRETER_START = @compileError("unable to translate C expr: unexpected token .Semicolon"); // ../oniguruma/src/regexec.c:2686:9
pub const CASE_OP = @compileError("unable to translate C expr: unexpected token .HashHash"); // ../oniguruma/src/regexec.c:2688:9
pub const GOTO_OP = @compileError("unable to translate C expr: unexpected token .Keyword_goto"); // ../oniguruma/src/regexec.c:2693:9
pub const INC_OP = @compileError("TODO postfix inc/dec expr"); // ../oniguruma/src/regexec.c:2715:9
pub const JUMP_OUT_WITH_SPREV_SET = @compileError("unable to translate C expr: unexpected token .Semicolon"); // ../oniguruma/src/regexec.c:2716:9
pub const JUMP_OUT = @compileError("unable to translate C expr: unexpected token .Semicolon"); // ../oniguruma/src/regexec.c:2717:9
pub const BREAK_OUT = @compileError("unable to translate C expr: unexpected token .Semicolon"); // ../oniguruma/src/regexec.c:2718:9
pub const CHECK_INTERRUPT_JUMP_OUT = @compileError("unable to translate C expr: unexpected token .Semicolon"); // ../oniguruma/src/regexec.c:2719:9
pub const MATCH_DEBUG_OUT = @compileError("unable to translate C expr: unexpected token .Nl"); // ../oniguruma/src/regexec.c:2766:9
pub const MATCH_AT_ERROR_RETURN = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2769:9
pub const MATCH_COUNTER_OUT = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/regexec.c:2773:9
pub const REGSET_MATCH_AND_RETURN_CHECK = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:4422:9
pub const MATCH_AND_RETURN_CHECK = @compileError("unable to translate C expr: unexpected token .Equal"); // ../oniguruma/src/regexec.c:5400:9
...
pub fn IS_MBC_WORD_ASCII_MODE(enc: anytype, s: anytype, end: anytype, mode: anytype) callconv(.Inline) @TypeOf(if (mode == 0) ONIGENC_IS_MBC_WORD(enc, s, end) else ONIGENC_IS_MBC_WORD_ASCII(enc, s, end)) {
    return if (mode == 0) ONIGENC_IS_MBC_WORD(enc, s, end) else ONIGENC_IS_MBC_WORD_ASCII(enc, s, end);
}
pub fn STACK_MEM_START(reg: anytype, idx: anytype) callconv(.Inline) @TypeOf(if (MEM_STATUS_AT(reg.*.push_mem_start, idx) != 0) STACK_AT(mem_start_stk[idx].i).*.u.mem.pstr else mem_start_stk[idx].s) {
    return if (MEM_STATUS_AT(reg.*.push_mem_start, idx) != 0) STACK_AT(mem_start_stk[idx].i).*.u.mem.pstr else mem_start_stk[idx].s;
}
pub fn STACK_MEM_END(reg: anytype, idx: anytype) callconv(.Inline) @TypeOf(if (MEM_STATUS_AT(reg.*.push_mem_end, idx) != 0) STACK_AT(mem_end_stk[idx].i).*.u.mem.pstr else mem_end_stk[idx].s) {
    return if (MEM_STATUS_AT(reg.*.push_mem_end, idx) != 0) STACK_AT(mem_end_stk[idx].i).*.u.mem.pstr else mem_end_stk[idx].s;
}
pub const HISTORY_TREE_INIT_ALLOC_SIZE = 8;
pub const RREGC_SIZE = @import("std").meta.sizeof(c_int) * (from.*.num_regs);
pub const STK_ALT_FLAG = 0x0001;
pub const STK_SUPER_ALT = STK_ALT_FLAG;
pub const STK_ALT = 0x0002 | STK_ALT_FLAG;
pub const STK_MEM_START = 0x0010;
pub const STK_MEM_END = 0x8030;
pub const STK_REPEAT_INC = 0x0040;
pub const STK_CALLOUT = 0x0070;
pub const STK_VOID = 0x0000;
pub const STK_EMPTY_CHECK_START = 0x3000;
pub const STK_EMPTY_CHECK_END = 0x5000;
pub const STK_MEM_END_MARK = 0x8100;
pub const STK_CALL_FRAME = 0x0400 | STK_MASK_POP_HANDLED;
pub const STK_RETURN = 0x0500 | STK_MASK_POP_HANDLED;
pub const STK_SAVE_VAL = 0x0600;
pub const STK_MARK = 0x0704;
pub const STK_MASK_POP_USED = STK_ALT_FLAG;
pub const STK_MASK_POP_HANDLED = 0x0010;
pub const STK_MASK_POP_HANDLED_TIL = STK_MASK_POP_HANDLED | 0x0004;
pub const STK_MASK_TO_VOID_TARGET = 0x100e;
pub const STK_MASK_MEM_END_OR_MARK = 0x8000;
pub const INVALID_STACK_INDEX = StackIndex - 1;
pub fn PTR_NUM_SIZE(reg: anytype) callconv(.Inline) @TypeOf(((reg.*.num_mem) + 1) * 2) {
    return ((reg.*.num_mem) + 1) * 2;
}
pub const ALLOCA_PTR_NUM_LIMIT = 50;
pub fn CALLOUT_DATA_AT_NUM(mp: anytype, num: anytype) callconv(.Inline) @TypeOf((mp.*.callout_data) + (num - 1)) {
    return (mp.*.callout_data) + (num - 1);
}
pub fn STACK_AT(index_1: anytype) callconv(.Inline) @TypeOf(stk_base + index_1) {
    return stk_base + index_1;
}
pub fn GET_STACK_INDEX(stk: anytype) callconv(.Inline) @TypeOf(stk - stk_base) {
    return stk - stk_base;
}
pub fn IS_TO_VOID_TARGET(stk: anytype) callconv(.Inline) @TypeOf(((stk.*.type) & STK_MASK_TO_VOID_TARGET) != 0) {
    return ((stk.*.type) & STK_MASK_TO_VOID_TARGET) != 0;
}
pub fn STACK_PUSH_ALT(pat: anytype, s: anytype) callconv(.Inline) @TypeOf(STACK_PUSH(STK_ALT, pat, s)) {
    return STACK_PUSH(STK_ALT, pat, s);
}
pub fn STACK_PUSH_SUPER_ALT(pat: anytype, s: anytype) callconv(.Inline) @TypeOf(STACK_PUSH(STK_SUPER_ALT, pat, s)) {
    return STACK_PUSH(STK_SUPER_ALT, pat, s);
}
pub fn STACK_PUSH_ALT_WITH_ZID(pat: anytype, s: anytype, id: anytype) callconv(.Inline) @TypeOf(STACK_PUSH_WITH_ZID(STK_ALT, pat, s, id)) {
    return STACK_PUSH_WITH_ZID(STK_ALT, pat, s, id);
}
pub fn GET_EMPTY_CHECK_START(sid: anytype, k: anytype) callconv(.Inline) @TypeOf(EMPTY_CHECK_START_SEARCH(sid, k)) {
    return EMPTY_CHECK_START_SEARCH(sid, k);
}
pub fn STACK_GET_REPEAT_COUNT(sid: anytype, c: anytype) callconv(.Inline) @TypeOf(STACK_GET_REPEAT_COUNT_SEARCH(sid, c)) {
    return STACK_GET_REPEAT_COUNT_SEARCH(sid, c);
}
pub const IS_EMPTY_STR = str == end;
pub fn ON_STR_BEGIN(s: anytype) callconv(.Inline) @TypeOf(s == str) {
    return s == str;
}
pub fn ON_STR_END(s: anytype) callconv(.Inline) @TypeOf(s == end) {
    return s == end;
}
pub const DATA_ENSURE_CHECK1 = s < right_range;
pub fn DATA_ENSURE_CHECK(n: anytype) callconv(.Inline) @TypeOf((s + n) <= right_range) {
    return (s + n) <= right_range;
}
pub const NEXT_OP = JUMP_OP;
pub const JUMP_OP = GOTO_OP;
pub const ACTIVATE_ALL_LOW_HIGH_SEARCH_THRESHOLD_LEN = 500;
pub const REGSET_INITIAL_ALLOC_SIZE = 10;
...
pub const _StackType = struct__StackType;
pub const SearchRangeStatus = enum_SearchRangeStatus;
