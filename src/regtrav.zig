...
pub fn onig_capture_tree_traverse(arg_region: [*c]OnigRegion, arg_at: c_int, arg_callback_func: ?fn (c_int, c_int, c_int, c_int, c_int, ?*c_void) callconv(.C) c_int, arg_arg: ?*c_void) callconv(.C) c_int {
    var region = arg_region;
    var at = arg_at;
    var callback_func = arg_callback_func;
    var arg = arg_arg;
    return capture_tree_traverse(region.*.history_root, at, callback_func, @as(c_int, 0), arg);
}
...
pub fn capture_tree_traverse(arg_node: [*c]OnigCaptureTreeNode, arg_at: c_int, arg_callback_func: ?fn (c_int, c_int, c_int, c_int, c_int, ?*c_void) callconv(.C) c_int, arg_level: c_int, arg_arg: ?*c_void) callconv(.C) c_int {
    var node = arg_node;
    var at = arg_at;
    var callback_func = arg_callback_func;
    var level = arg_level;
    ...
    if (node == @intToPtr([*c]OnigCaptureTreeNode, @as(c_int, 0))) return 0;
    if ((at & @as(c_int, 1)) != @as(c_int, 0)) {
        r = (callback_func).?(node.*.group, node.*.beg, node.*.end, level, @as(c_int, 1), arg);
        if (r != @as(c_int, 0)) return r;
    }
    {
        i = 0;
        while (i < node.*.num_childs) : (i += 1) {
            r = capture_tree_traverse(node.*.childs[@intCast(c_uint, i)], at, callback_func, (level + @as(c_int, 1)), arg);
            if (r != @as(c_int, 0)) return r;
        }
    }
    if ((at & @as(c_int, 2)) != @as(c_int, 0)) {
        r = (callback_func).?(node.*.group, node.*.beg, node.*.end, level, @as(c_int, 2), arg);
        if (r != @as(c_int, 0)) return r;
    }
    return 0;
}
