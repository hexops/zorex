...
pub const EncLen_EUCTW: [256]c_int = [256]c_int{
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
    @as(c_int, 1),
...
pub fn euctw_mbc_enc_len(arg_p: [*c]const OnigUChar) callconv(.C) c_int {
    var p = arg_p;
    return EncLen_EUCTW[p.?.*];
}
pub fn euctw_code_to_mbclen(arg_code: OnigCodePoint) callconv(.C) c_int {
    var code = arg_code;
    if ((code & @as(c_uint, 4278190080)) != @bitCast(c_uint, @as(c_int, 0))) {
        if (EncLen_EUCTW[@intCast(c_uint, (@bitCast(c_int, (code >> @intCast(@import("std").math.Log2Int(OnigCodePoint), 24))) & @as(c_int, 255)))] == @as(c_int, 4)) return 4;
        ...
        if (EncLen_EUCTW[@intCast(c_uint, (@bitCast(c_int, (code >> @intCast(@import("std").math.Log2Int(OnigCodePoint), 8))) & @as(c_int, 255)))] == @as(c_int, 2)) return 2;
    } else {
        if (EncLen_EUCTW[@intCast(c_uint, @bitCast(c_int, (code & @bitCast(c_uint, @as(c_int, 255)))))] == @as(c_int, 1)) return 1;
    }
    return -@as(c_int, 400);
}
pub fn is_valid_mbc_string(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    ...
    while (p < end) {
        if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 128)) {
            ...
            if (@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 142)) {
                p += 1;
                if (p >= end) return 0;
                if ((@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 161)) or (@bitCast(c_int, @as(c_uint, p.?.*)) > @as(c_int, 176))) return 0;
                p += 1;
                if (p >= end) return 0;
        ...
        } else if (@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 255)) {
            p += 1;
            if (p >= end) return 0;
            if ((@bitCast(c_int, @as(c_uint, p.?.*)) < @as(c_int, 161)) or (@bitCast(c_int, @as(c_uint, p.?.*)) == @as(c_int, 255))) return 0;
            p += 1;
...
pub fn euctw_mbc_to_code(arg_p: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) OnigCodePoint {
    var p = arg_p;
    var end = arg_end;
    return onigenc_mbn_mbc_to_code((&OnigEncodingEUC_TW), p, end);
}
pub fn euctw_code_to_mbc(arg_code: OnigCodePoint, arg_buf: [*c]OnigUChar) callconv(.C) c_int {
    var code = arg_code;
    var buf = arg_buf;
    return onigenc_mb4_code_to_mbc((&OnigEncodingEUC_TW), code, buf);
}
pub fn euctw_mbc_case_fold(arg_flag: OnigCaseFoldType, arg_pp: [*c][*c]const OnigUChar, arg_end: [*c]const OnigUChar, arg_lower: [*c]OnigUChar) callconv(.C) c_int {
    var flag = arg_flag;
    var pp = arg_pp;
    var end = arg_end;
    var lower = arg_lower;
    return onigenc_mbn_mbc_case_fold((&OnigEncodingEUC_TW), flag, pp, end, lower);
}
pub fn euctw_is_code_ctype(arg_code: OnigCodePoint, arg_ctype: c_uint) callconv(.C) c_int {
    var code = arg_code;
    var ctype = arg_ctype;
    return onigenc_mb4_is_code_ctype((&OnigEncodingEUC_TW), code, ctype);
}
pub fn euctw_left_adjust_char_head(arg_start: [*c]const OnigUChar, arg_s: [*c]const OnigUChar) callconv(.C) [*c]OnigUChar {
    var start = arg_start;
    var s = arg_s;
    ...
    len = (&OnigEncodingEUC_TW).*.mbc_enc_len.?(p);
...
pub fn euctw_is_allowed_reverse_match(arg_s: [*c]const OnigUChar, arg_end: [*c]const OnigUChar) callconv(.C) c_int {
    var s = arg_s;
    var end = arg_end;
    const c: OnigUChar = s.?.*;
    if (@bitCast(c_int, @as(c_uint, c)) <= @as(c_int, 126)) return 1 else return 0;
    return 0;
}
...
pub fn euctw_islead(c: anytype) callconv(.Inline) @TypeOf((@import("std").meta.cast(UChar, c - 0xa1)) > (0xfe - 0xa1)) {
