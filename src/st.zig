...
pub const struct_st_table_entry = extern struct {
    hash: c_uint,
    key: st_data_t,
    record: st_data_t,
    next: [*c]st_table_entry,
};
pub const struct_st_table = extern struct {
    type: [*c]struct_st_hash_type,
    num_bins: c_int,
    num_entries: c_int,
    bins: [*c][*c]struct_st_table_entry,
};
...
pub export fn onig_st_init_table(arg_type_1: [*c]struct_st_hash_type) [*c]st_table {
    var type_1 = arg_type_1;
    return onig_st_init_table_with_size(type_1, @as(c_int, 0));
}
pub export fn onig_st_init_table_with_size(arg_type_1: [*c]struct_st_hash_type, arg_size: c_int) [*c]st_table {
    var type_1 = arg_type_1;
    var size = arg_size;
    var tbl: [*c]st_table = undefined;
    size = new_size(size);
    if (size <= @as(c_int, 0)) return null;
    tbl = @ptrCast([*c]st_table, @alignCast(@alignOf(st_table), malloc(@bitCast(c_ulong, @as(c_ulong, @bitCast(c_uint, @truncate(c_uint, @sizeOf(st_table))))))));
    if (tbl == null) return null;
    tbl.*.type = type_1;
    tbl.*.num_entries = 0;
    tbl.*.num_bins = size;
    tbl.*.bins = @ptrCast([*c][*c]st_table_entry, @alignCast(@alignOf([*c]st_table_entry), @ptrCast([*c]u8, @alignCast(@alignOf(u8), calloc(@bitCast(c_ulong, @as(c_long, (size))), (@sizeOf([*c]st_table_entry)))))));
    if (tbl.*.bins == null) {
        free(@ptrCast(?*c_void, tbl));
        return null;
    }
    return tbl;
}
pub export fn onig_st_init_numtable() [*c]st_table {
    return onig_st_init_table(&type_numhash);
}
pub export fn onig_st_init_numtable_with_size(arg_size: c_int) [*c]st_table {
    var size = arg_size;
    return onig_st_init_table_with_size(&type_numhash, size);
}
pub export fn onig_st_init_strtable() [*c]st_table {
    return onig_st_init_table(&type_strhash);
}
pub export fn onig_st_init_strtable_with_size(arg_size: c_int) [*c]st_table {
    var size = arg_size;
    return onig_st_init_table_with_size(&type_strhash, size);
}
pub export fn onig_st_delete(arg_table: [*c]st_table, arg_key: [*c]st_data_t, arg_value: [*c]st_data_t) c_int {
    var table = arg_table;
    var key = arg_key;
    var value = arg_value;
    var hash_val: c_uint = undefined;
    var tmp: [*c]st_table_entry = undefined;
    var ptr: [*c]st_table_entry = undefined;
    hash_val = (@bitCast(c_uint, ((table).*.type.*.hash).?((key.?.*))) % @bitCast(c_uint, (table).*.num_bins));
    ptr = table.*.bins[hash_val];
    if (ptr == null) {
        if (value != null) value.?.* = @bitCast(st_data_t, @as(c_long, @as(c_int, 0)));
        return 0;
    }
    if (((key.?.*) == (ptr.*.key)) or ((table.*.type.*.compare).?((key.?.*), (ptr.*.key)) == @as(c_int, 0))) {
    ...
    while (ptr.*.next != null) : (ptr = ptr.*.next) {
        if (((ptr.*.next.*.key) == (key.?.*)) or ((table.*.type.*.compare).?((ptr.*.next.*.key), (key.?.*)) == @as(c_int, 0))) {
            tmp = ptr.*.next;
            ptr.*.next = ptr.*.next.*.next;
            table.*.num_entries -= 1;
            if (value != null) value.?.* = tmp.*.record;
            key.?.* = tmp.*.key;
            free(@ptrCast(?*c_void, tmp));
            return 1;
        }
    }
    return 0;
}
pub export fn onig_st_delete_safe(arg_table: [*c]st_table, arg_key: [*c]st_data_t, arg_value: [*c]st_data_t, arg_never: st_data_t) c_int {
    var table = arg_table;
    var key = arg_key;
    var value = arg_value;
    var never = arg_never;
    ...
    while (ptr != null) : (ptr = ptr.*.next) {
        if ((ptr.*.key != never) and (((ptr.*.key) == (key.?.*)) or ((table.*.type.*.compare).?((ptr.*.key), (key.?.*)) == @as(c_int, 0)))) {
            table.*.num_entries -= 1;
            key.?.* = ptr.*.key;
            if (value != null) value.?.* = ptr.*.record;
            ptr.*.key = blk: {
...
pub export fn onig_st_insert(arg_table: [*c]st_table, arg_key: st_data_t, arg_value: st_data_t) c_int {
    var table = arg_table;
    var key = arg_key;
    var value = arg_value;
    var hash_val: c_uint = undefined;
    var bin_pos: c_uint = undefined;
    var ptr: [*c]st_table_entry = undefined;
    hash_val = @bitCast(c_uint, ((table).*.type.*.hash).?((key)));
    while (true) {
        bin_pos = (hash_val % @bitCast(c_uint, (table).*.num_bins));
        ptr = (table).*.bins[bin_pos];
        if (((ptr) != null) and ((ptr.*.hash != (hash_val)) or !(((key) == ((ptr).*.key)) or (((table).*.type.*.compare).?((key), ((ptr).*.key)) == @as(c_int, 0))))) {
            {}
            while (((ptr.*.next) != null) and ((ptr.*.next.*.hash != (hash_val)) or !(((key) == ((ptr.*.next).*.key)) or (((table).*.type.*.compare).?((key), ((ptr.*.next).*.key)) == @as(c_int, 0))))) {
                ptr = ptr.*.next;
            }
            ptr = ptr.*.next;
        }
    ...
    if (ptr == null) {
        while (true) {
            var entry: [*c]st_table_entry = undefined;
            if (@divTrunc(table.*.num_entries, (table.*.num_bins)) > @as(c_int, 5)) {
                rehash(table);
                bin_pos = (hash_val % @bitCast(c_uint, table.*.num_bins));
            }
            entry = @ptrCast([*c]st_table_entry, @alignCast(@alignOf(st_table_entry), malloc(@bitCast(c_ulong, @as(c_ulong, @bitCast(c_uint, @truncate(c_uint, @sizeOf(st_table_entry))))))));
            if ((@ptrCast(?*c_void, (entry))) == @intToPtr(?*c_void, @as(c_int, 0))) return -@as(c_int, 5);
            entry.*.hash = hash_val;
            entry.*.key = key;
            entry.*.record = value;
...
pub export fn onig_st_lookup(arg_table: [*c]st_table, arg_key: st_data_t, arg_value: [*c]st_data_t) c_int {
    var table = arg_table;
    var key = arg_key;
    var value = arg_value;
    var hash_val: c_uint = undefined;
...
pub export fn onig_st_foreach(arg_table: [*c]st_table, arg_func: ?fn (...) callconv(.C) c_int, arg_arg: st_data_t) c_int {
    var table = arg_table;
    var func = arg_func;
    var arg = arg_arg;
    var ptr: [*c]st_table_entry = undefined;
    var last: [*c]st_table_entry = undefined;
    var tmp: [*c]st_table_entry = undefined;
    var retval: enum_st_retval = undefined;
    var i: c_int = undefined;
    {
        i = 0;
        while (i < table.*.num_bins) : (i += 1) {
            last = null;
            {
                ptr = table.*.bins[@intCast(c_uint, i)];
                while (ptr != null) {
                    retval = @intToEnum(enum_st_retval, (func).?(ptr.*.key, ptr.*.record, arg));
                    @"switch": {
                        case_3: {
                            case_2: {
                                case_1: {
                                    case: {
                                        switch (@bitCast(c_uint, @enumToInt(retval))) {
                                            @bitCast(c_uint, @as(c_int, 3)) => break :case,
                                            @bitCast(c_uint, @as(c_int, 0)) => break :case_1,
                                    ...
                                    if (i < table.*.num_bins) {
                                        {
                                            tmp = table.*.bins[@intCast(c_uint, i)];
                        ...
                        tmp = ptr;
                        if (last == null) {
                            table.*.bins[@intCast(c_uint, i)] = ptr.*.next;
                        } else {
                            last.*.next = ptr.*.next;
                        }
                        ptr = ptr.*.next;
...
pub export fn onig_st_add_direct(arg_table: [*c]st_table, arg_key: st_data_t, arg_value: st_data_t) void {
    var table = arg_table;
    var key = arg_key;
    var value = arg_value;
    var hash_val: c_uint = undefined;
    ...
    while (true) {
        ...
        entry = @ptrCast([*c]st_table_entry, @alignCast(@alignOf(st_table_entry), malloc(@bitCast(c_ulong, @as(c_ulong, @bitCast(c_uint, @truncate(c_uint, @sizeOf(st_table_entry))))))));
        if ((@ptrCast(?*c_void, (entry))) == @intToPtr(?*c_void, @as(c_int, 0))) return;
        entry.*.hash = hash_val;
        entry.*.key = key;
        entry.*.record = value;
        entry.*.next = table.*.bins[bin_pos];
...
pub export fn onig_st_free_table(arg_table: [*c]st_table) void {
    var table = arg_table;
    var ptr: [*c]st_table_entry = undefined;
    var next: [*c]st_table_entry = undefined;
    var i: c_int = undefined;
    {
        i = 0;
        while (i < table.*.num_bins) : (i += 1) {
    ...
    free(@ptrCast(?*c_void, table.*.bins));
    free(@ptrCast(?*c_void, table));
}
pub export fn onig_st_cleanup_safe(arg_table: [*c]st_table, arg_never: st_data_t) void {
    var table = arg_table;
    var never = arg_never;
    var num_entries: c_int = table.*.num_entries;
    _ = onig_st_foreach(table, @ptrCast(?fn (...) callconv(.C) c_int, @alignCast(@alignOf(fn (...) callconv(.C) c_int), delete_never)), never);
    table.*.num_entries = num_entries;
}
pub export fn onig_st_copy(arg_old_table: [*c]st_table) [*c]st_table {
    var old_table = arg_old_table;
    var new_table: [*c]st_table = undefined;
    var ptr: [*c]st_table_entry = undefined;
    var entry: [*c]st_table_entry = undefined;
    var i: c_int = undefined;
    var num_bins: c_int = old_table.*.num_bins;
    new_table = @ptrCast([*c]st_table, @alignCast(@alignOf(st_table), malloc(@bitCast(c_ulong, @as(c_ulong, @bitCast(c_uint, @truncate(c_uint, @sizeOf(st_table))))))));
    if (new_table == null) {
        return null;
    }
    new_table.?.* = old_table.?.*;
    new_table.*.bins = @ptrCast([*c][*c]st_table_entry, @alignCast(@alignOf([*c]st_table_entry), @ptrCast([*c]u8, @alignCast(@alignOf(u8), calloc(@bitCast(c_ulong, @as(c_ulong, (@bitCast(c_uint, num_bins)))), (@sizeOf([*c]st_table_entry)))))));
    if (new_table.*.bins == null) {
        free(@ptrCast(?*c_void, new_table));
        return null;
    }
    {
        i = 0;
        while (i < num_bins) : (i += 1) {
            new_table.*.bins[@intCast(c_uint, i)] = null;
            ptr = old_table.*.bins[@intCast(c_uint, i)];
            while (ptr != null) {
                entry = @ptrCast([*c]st_table_entry, @alignCast(@alignOf(st_table_entry), malloc(@bitCast(c_ulong, @as(c_ulong, @bitCast(c_uint, @truncate(c_uint, @sizeOf(st_table_entry))))))));
                if (entry == null) {
                    free(@ptrCast(?*c_void, new_table.*.bins));
                    free(@ptrCast(?*c_void, new_table));
                    return null;
                }
                entry.?.* = ptr.?.*;
                entry.*.next = new_table.*.bins[@intCast(c_uint, i)];
                new_table.*.bins[@intCast(c_uint, i)] = entry;
                ptr = ptr.*.next;
            }
        }
    }
...
pub fn numcmp(arg_x: c_long, arg_y: c_long) callconv(.C) c_int {
    var x = arg_x;
    var y = arg_y;
    return @boolToInt((x != y));
}
pub fn numhash(arg_n: c_long) callconv(.C) c_int {
    var n = arg_n;
    return @bitCast(c_int, @truncate(c_int, n));
}
pub var type_numhash: struct_st_hash_type = struct_st_hash_type{
    .compare = @ptrCast(?fn (...) callconv(.C) c_int, @alignCast(@alignOf(fn (...) callconv(.C) c_int), numcmp)),
    .hash = @ptrCast(?fn (...) callconv(.C) c_int, @alignCast(@alignOf(fn (...) callconv(.C) c_int), numhash)),
};
pub fn strhash(arg_string: [*c]const u8) callconv(.C) c_int {
    var string = arg_string;
    var c: c_int = undefined;
    var val: c_int = 0;
    while ((blk: {
        const tmp = @bitCast(c_int, @as(c_uint, (blk_1: {
            const ref = &string;
            const tmp_2 = ref.*;
            ref.* += 1;
            break :blk_1 tmp_2;
        ...
        val = ((val * @as(c_int, 997)) + c);
    }
    return (val + (val >> @intCast(@import("std").math.Log2Int(c_int), 5)));
}
pub var type_strhash: struct_st_hash_type = struct_st_hash_type{
    .compare = @ptrCast(?fn (...) callconv(.C) c_int, @alignCast(@alignOf(fn (...) callconv(.C) c_int), strcmp)),
    .hash = @ptrCast(?fn (...) callconv(.C) c_int, @alignCast(@alignOf(fn (...) callconv(.C) c_int), strhash)),
};
pub fn rehash(arg_table: [*c]st_table) callconv(.C) void {
    var table = arg_table;
    var ptr: [*c]st_table_entry = undefined;
    var next: [*c]st_table_entry = undefined;
    var new_bins: [*c][*c]st_table_entry = undefined;
    var i: c_int = undefined;
    var new_num_bins: c_int = undefined;
    var old_num_bins: c_int = undefined;
    var hash_val: c_uint = undefined;
    old_num_bins = table.*.num_bins;
    new_num_bins = new_size((old_num_bins + @as(c_int, 1)));
    if (new_num_bins <= @as(c_int, 0)) return;
    new_bins = @ptrCast([*c][*c]st_table_entry, @alignCast(@alignOf([*c]st_table_entry), @ptrCast([*c]u8, @alignCast(@alignOf(u8), calloc(@bitCast(c_ulong, @as(c_long, (new_num_bins))), (@sizeOf([*c]st_table_entry)))))));
    if (new_bins == null) {
        return;
    }
    {
        i = 0;
        while (i < old_num_bins) : (i += 1) {
            ptr = table.*.bins[@intCast(c_uint, i)];
            while (ptr != null) {
                next = ptr.*.next;
                hash_val = (ptr.*.hash % @bitCast(c_uint, new_num_bins));
...
pub const primes: [29]c_long = [29]c_long{
    @bitCast(c_long, @as(c_long, (@as(c_int, 8) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 16) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 32) + @as(c_int, 5)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 64) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 128) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 256) + @as(c_int, 27)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 512) + @as(c_int, 9)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 1024) + @as(c_int, 9)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 2048) + @as(c_int, 5)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 4096) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 8192) + @as(c_int, 27)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 16384) + @as(c_int, 43)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 32768) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 65536) + @as(c_int, 45)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 131072) + @as(c_int, 29)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 262144) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 524288) + @as(c_int, 21)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 1048576) + @as(c_int, 7)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 2097152) + @as(c_int, 17)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 4194304) + @as(c_int, 15)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 8388608) + @as(c_int, 9)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 16777216) + @as(c_int, 43)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 33554432) + @as(c_int, 35)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 67108864) + @as(c_int, 15)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 134217728) + @as(c_int, 29)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 268435456) + @as(c_int, 3)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 536870912) + @as(c_int, 11)))),
    @bitCast(c_long, @as(c_long, (@as(c_int, 1073741824) + @as(c_int, 85)))),
    @bitCast(c_long, @as(c_long, @as(c_int, 0))),
};
pub fn new_size(arg_size: c_int) callconv(.C) c_int {
    var size = arg_size;
    var i: c_int = undefined;
    var newsize: c_int = undefined;
    {
        ...
        while (i < @bitCast(c_int, @truncate(c_uint, (@sizeOf([29]c_long) / @sizeOf(c_long))))) : (_ = (blk: {
            i += 1;
            break :blk (blk_1: {
                const ref = &newsize;
            ...
            if (newsize > size) return @bitCast(c_int, @truncate(c_int, primes[@intCast(c_uint, i)]));
        }
    }
    return -@as(c_int, 1);
}
pub fn delete_never(arg_key: st_data_t, arg_value: st_data_t, arg_never: st_data_t) callconv(.C) c_int {
    var key = arg_key;
    var value = arg_value;
    var never = arg_never;
...
pub const FIND_ENTRY = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/st.c:232:9
pub const ADD_DIRECT = @compileError("unable to translate C expr: unexpected token .Keyword_do"); // ../oniguruma/src/st.c:265:9
...
pub const ST_DEFAULT_MAX_DENSITY = 5;
pub const ST_DEFAULT_INIT_TABLE_SIZE = 11;
pub fn alloc(type_1: anytype) callconv(.Inline) @TypeOf((@import("std").meta.cast([*c]type_1, xmalloc((@import("std").meta.cast(c_uint, @import("std").meta.sizeof(type_1))))))) {
    return (@import("std").meta.cast([*c]type_1, xmalloc((@import("std").meta.cast(c_uint, @import("std").meta.sizeof(type_1))))));
}
pub fn Calloc(n: anytype, s: anytype) callconv(.Inline) @TypeOf((@import("std").meta.cast([*c]u8, xcalloc(n, s)))) {
    return (@import("std").meta.cast([*c]u8, xcalloc(n, s)));
}
pub fn EQUAL(table: anytype, x: anytype, y: anytype) callconv(.Inline) @TypeOf((x == y) or ((@import("std").meta.cast((table.*.type.*.compare).*, blk: {
    _ = x;
    break :blk y;
})) == 0)) {
    return (x == y) or ((@import("std").meta.cast((table.*.type.*.compare).*, blk: {
        _ = x;
        break :blk y;
    })) == 0);
}
pub fn do_hash(key: anytype, table: anytype) callconv(.Inline) @TypeOf((@import("std").meta.cast(c_uint, (table.*.type.*.hash).*))(key)) {
    return (@import("std").meta.cast(c_uint, (table.*.type.*.hash).*))(key);
}
pub fn do_hash_bin(key: anytype, table: anytype) callconv(.Inline) @TypeOf(do_hash(key, table) % (table.*.num_bins)) {
    return do_hash(key, table) % (table.*.num_bins);
}
pub const MINSIZE = 8;
pub fn PTR_NOT_EQUAL(table: anytype, ptr: anytype, hash_val: anytype, key: anytype) callconv(.Inline) @TypeOf((ptr != 0) and (((ptr.*.hash) != hash_val) or !(EQUAL(table, key, ptr.*.key) != 0))) {
    return (ptr != 0) and (((ptr.*.hash) != hash_val) or !(EQUAL(table, key, ptr.*.key) != 0));
}
